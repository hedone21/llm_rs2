...
- **Adreno 디바이스 권장 backend = `qnn_oppkg`** (Galaxy S25 검증, fast path 미사용 시에도). KV cache가 dual buffer (rpcmem DMA-BUF heap + OpenCL `CL_MEM_USE_HOST_PTR` alias)로 alloc되어 OpenCL `read/write_buffer`가 zero-copy화. Qwen2.5-1.5B Q4_0 실측 Decode TBT: `--backend opencl` 28 ms/tok vs `--backend qnn_oppkg` (fast off) 13 ms/tok = 2× 가속, llama.cpp(Adreno OpenCL) 27 ms/tok 대비도 2×. **`CL_MEM_ALLOC_HOST_PTR` (UnifiedBuffer / `--zero-copy` flag)는 Adreno에서 zero-copy 효과 없음** (driver internal pool alloc) — 진짜 가속 원천은 DMA-BUF interop. `--backend qnn_oppkg`는 `libQnnGpu.so`+`libqnn_oppkg.so`+`libcdsprpc.so` 필요 (Galaxy S25에 사전 배포). fast path on (`LLMRS_QNN_OPPKG_FAST_PATH=1`)은 현재 graph-side bridge memcpy로 79 ms/tok (Step 2 graph KV mem_handle 외부 주입 작업 대기). **`--tokenizer-path` 명시 필수** — 자동 fallback이 sibling 모델의 tokenizer를 잡으면 vocab-size mismatch error로 차단 (commit 45cb489).

## QCF 명명 컨벤션 (2026-04-27 결정, rename 미적용 — TODO)

QCF(Quality Cost Function)는 두 패밀리로 명확히 구분한다. 액션마다 측정 공간이 달라 단일 통일이 불가하므로, **이름으로 패밀리를 명시**한다.

| 패밀리 | 측정 공간 | 포함 액션 | 코드 위치 (현재 명) |
|---|---|---|---|
| **QCF_kv** | KV 캐시 → attention output `‖ΔO‖₂ / ‖O‖₂` | sliding/H2O/streaming eviction, KIVI quant, D2O merge | `core/qcf/unified_qcf.rs` (구 unified) |
| **QCF_weight** | 모델 forward path (weight/layer 단위) | weight swap (F16→Q4), layer skip (SWIFT) | `core/qcf/decider.rs::compute_qcf_swap`, `core/qcf/layer_importance.rs::compute_qcf` |
...
- **rename 작업은 backlog에 등록**: `.agent/todos/backlog.md` "QCF 명명 컨벤션 정리" 항목. 코드 rename + estimator key 정규화(`kv.*` / `weight.*` prefix) + 문서 갱신.
