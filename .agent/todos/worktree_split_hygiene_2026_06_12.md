...
> **⚠ 효력 종료 (2026-06-12 사용자 결정)**: 별도 워크트리 리팩토링을 **하지 않기로 확정** — 본 문서의 ③ rename 상호 배제(R1·R2 동결)와 머지 판정 용도는 효력 종료. KV 로드맵 항목 2·4-impl·5 의 "리팩토링 머지 후" 게이트 해제(즉시 착수 가능). ② 의 게이트 3항(host lib / α-K frozen / S25 verify — **이제 30/30**, QCF_kv 라운드 종결)은 일반 회귀 게이트 정의로 계속 참조 가치 있음.
