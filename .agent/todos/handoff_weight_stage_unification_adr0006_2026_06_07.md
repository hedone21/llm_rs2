...
> **MW-A·B·C ✅ 완료** (Seam A 3/4). plugin 표면(`WeightStage`/`WeightStageCtx`/`WEIGHT_STAGES`)이 실재하고, 빌트인 `WeightSwapDeciderAsStage`("swap")가 그 표면을 **소비**해 검증됨(전략 C — decider 를 flat &[f32]+count 로 정렬, engine-internal trait 결합 제거). 남은 것 = **MW-D**(엔진 ctx impl)뿐. **MW-D 미해결 설계점**: ctx.budget() 의 ratio 원천 — Seam B(EngineCommand::SwapWeights{ratio}) 부재 시, ctx 를 budget(count) 보유 struct 로 만들지(테스트 구성) 결정 필요. **주의**: spec 타겟에 pre-existing 실패 6건(stash-verify 확인) — inv_layer_001/003/006(htp_fastrpc.rs/opencl.rs cross-backend import, 내 변경 무관) + GPU 부재 3건(No OpenCL devices). `engine/Cargo.toml` 의 microbench_score_readback bin 항목은 pre-existing 미커밋(건드리지/커밋하지 말 것).
...
- **MW-C ⏸ BLOCKED (설계 결정 대기)** — **`WeightSwapDeciderAsStage` 어댑터**가 막혔다. api `WeightStageCtx`(D4: flat `layer_metric` &[f32] + `budget`=선해소 count)가 기존 `WeightSwapDecider`(`&dyn ImportanceLookup` entries-based + `&dyn QuantNoiseAccess` + `decide(ratio)` 내부 재해소)와 **3겹 불일치**: importance flat↔entries, noise flat↔as_slice(경미), budget-as-count ↔ decide의 ratio-내부해소(이중 해소 위험). **D1("ctx borrow 로 decider 생성") ↔ D4(flat ctx) 내부 충돌** + ADR이 명시한 "weight OCP 미실증(decider=engine-internal trait 3종 결합)"의 실체. 결정 fork: **(A) builtin 이 api ctx 소비(shim importance→ImportanceLookup/noise→QuantNoiseAccess + budget→ratio) → 표면 검증, decider 무변경 / (B) builtin 이 api ctx 우회(concrete ctx downcast/model refs) → ADR builtin-first 부합, 표면 미실증 유지 / (C) decider 를 flat &[f32]+count 로 리팩터 → 표면 자연 입력화, but 12 call site(9 test+3 prod) 침습**. **결정 = C** (사용자 승인). decider 를 flat `&[f32]`+count 로 리팩터 → api ctx 가 decider 자연 입력화(plugin 표면 실증). bit-identical 보존 키: (1) `scored_path_available` 의 `noise.is_computed()` → `noise.is_some()` (ctx 계약: `layer_metric(QuantNoise)=Some ⟺ computed`, 엔진 ctx 미산출 시 None). (2) importance 결측 디폴트 ranking=0.0 / QCF=1.0 비대칭 보존(flat 투영 전 레이어 채우면 무영향). (3) ε ranking 미사용·QCF 만 사용 보존. (4) `ratio→count`(`needed=floor(ratio·n)−|swapped|`) 를 decide 밖(호출자/ctx.budget)으로 이동 → `decide(budget:usize)`. 게이트 = 9 decider 테스트 bit-identical(flat 입력 재작성, 동일 expected) + lib ≥1238/0 + 어댑터 테스트(plan()==decide) + clippy --workspace. **senior-implementer 위임(백그라운드), 완료 시 메인이 diff 검토+커밋.** → **✅ DONE `5a584e71`**: 전략 C 구현 완료. decider flat 리팩터(bit-identical, 13 decider + 4 spec 통합 테스트 재작성, assert 무변경) + `WeightSwapDeciderAsStage`("swap" 등록, swap=`LayerDirective{Full, precision:Some(Q4_0)}` → precision⊥dispatch R1 실행) + `ensure_builtin_weight_stages_registered`. 게이트: 전 타겟 컴파일 + lib 1244/0 + clippy/build clean + 우리 spec 30/30.
