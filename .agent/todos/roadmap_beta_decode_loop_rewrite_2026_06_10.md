...
### β-5 — Pressure 일원화 (Level→Pressure+band) — ✅ 완료 2026-06-10

> **완료 기록**: `8dc2aff9`(ripple — determine_pressure_level → `Pressure::from_mem_available().band()` 일원화. **실측 정정: PressureLevel 은 기왕 `pub type PressureLevel = llm_shared::Level` 별칭**(pressure.rs:45)이라 roadmap 의 "10파일 ripple" 은 별칭 유지로 행위 변경 2파일에 수렴 — handler 4종/chat/tests 는 variant 접근뿐이라 무변경. 미기재 사용처 1건 추가 발견: test_eng_alg_211) / `5c7c9e03`(LocalPressureSource 신설(canonical cutoff 소유, memory-only) + DecodeLoop pressure_source 주입(N=8 step 캐시 — 거처는 DecodeLoop, source 는 immutable) + EvictionStage `persistent()` + beta5 게이트 테스트) / `f77342bb`(**오케스트레이터 검수 적발 2건 보완**: ① Persistent 매 step 재발화 → floor 나선 축소 퇴행 — **episode edge-trigger**(상향 에지 1회, band 하강 re-arm) 도입, ② persistent production 미제출 — build_bench_loop 에 shared_cm 재구조 + CM·source 양존 시 Persistent(Warning, CLI ratio) registry 제출). **host**: 경계값 전수 unit(4-level × t/t÷2/t÷4 ±1, threshold 5종 구==신 전사) + beta5 pressure-driven 등가 6/6(Warning Persistent 발화 == v1 force_evict anchor, 3정책×3dtype) + edge-trigger unit + 同코드 unit(Persistent==OneShot byte-identical) + beta3 10/10·beta4 5/5 유지 + lib 1313 PASS(23 FAIL=GPU 전수) + `cargo test --no-run` + fmt/clippy clean. **device(S25)**: happy-path sig 3/3 MATCH(무접촉 증명 — build_standard_loop diff 0) + **pressure-driven eviction 1회: sig `c41930a5..` == β-3 frozen baseline byte-identical** + CacheEvent `removed=459, new_pos=459` MATCH (threshold 65536MB → Emergency band → step 0 edge-fire, 동일 ratio/policy 라 command-driven 과 완전 동일 산출 — frozen 대비 최강 형태 통과). G4 defer: ManagerPressureSource = seam(PressureSource trait)만, 신규 코드 0.
> ★다음 착수 = **β-6**.
...
### β-6 — chat 수렴 (run_until_stop 통합) — ✅ 완료 2026-06-10

> **완료 기록**: commit A `aac35bd7`(거동 고정 핀 — stop 토큰 tick/obs 발화·pos-증가-후 체크·turn-boundary try_evict 직접 호출 보존) / commit B `c4544e02`(run_steps core 추출 + run()=core+finalize / run_until_stop=TurnStart+core(∞)+TurnEnd, push 를 DecodeEnd dispatch 뒤로 재배열(Stop 시 미push = chat 시맨틱), StepInfo `prev_token` 승격, ChatStopStage 신설, ChatSession 3종+new_for_test registry 이식) / commit C `15c32bc3`(TickStage(PostSample) 신설 + (f2) tick_sink 슬롯 제거 + with_resilience→build() submit + heartbeat 채널 보존). **설계 정정 2건**: ① ChatStopStage = **DecodeEnd 구독**(roadmap 의 "PostSample" 정정 — v1 은 stop 토큰에도 tick/obs 발화+pos 증가 후 check 라 DecodeEnd 의 ctx(prev_token=sampled, pos=증가됨)가 v1 `should_stop(sampled, pos)` 와 정확 일치; PostSample 이면 chat spec final_pos 단언 깨짐). ② turn별 동적 stop condition(repl.rs max_new_tokens) 때문에 빌드 시점 고정 불가 → **ChatStopSlot(공유 슬롯)+RAII ChatStopGuard** — `run_turn(&dyn StopCondition)` 외부 시그니처 보존, lifetime erasure unsafe 1곳(단일 스레드 INV-018 + guard drop=clear + panic-unwind 안전, 오케스트레이터 검수 수용 — mem::forget 오용은 내부 API 한정 이론 위험으로 기록). **host**: chat spec 20/20 + commit A 핀 전부 + inv_decode_stage 11/11 + beta3 10/10·beta4 6/6(tick 등가 신규 포함)·beta5 6/6 + lib 1319 PASS(22 FAIL=opencl 전수) + fmt/clippy clean. **device(S25)**: sig 15/15 MATCH + tbt f16 **54.56**(Δ+0.63%)/f32 **54.03**(Δ−0.02%)/q4 **53.44**(Δ−0.65%) + resilience-on smoke(Throttle 86.95 ms/tok 적용 + heartbeat 2건 — TickStage 경유 채널 생존).
> ★다음 착수 = **β-7**.
...
### β-7 — v1 표면 삭제 (순수 삭제 단독 편성, 최종 게이트) — ✅ 완료 2026-06-10

> **완료 기록**: `ab9189c6`(① 이동 — Forward→session/forward.rs(슬림 seam)/StepCtx·TokenSampler·GreedySampler→inference/sampling.rs(front-door ①)/**StopReason(5-variant)+DecodeResult→decode_loop.rs**(오케스트레이터 보정: StopFlag 는 stage 어휘(v2 4-variant) 표현 불가 — driver-result 어휘로 존치, 분리 사유 doc)/CommandSource·EngineReport→command_dispatcher.rs) / `1096db5a`(② 순수 삭제 — traits.rs 파일 + EvictionStage/SwapStage/TokenTickSink/ResilienceBundle/DecodeObserver/EvictionOutcome/SkipReason + run 슬롯 (b)(c)(e)(g) + 필드 4종 + NoOp 5종 + ReportWrapper. **소비자-0 grep 증빙**: DecodeObserver 비-NoOp impl 0·add_observer 호출처 = 테스트 2건뿐·report 슬롯 dead) / `a9e0a3de`(③ doc 정정). **typestate builder 보존**(최소 침습 — INV-LAYER-007 compile_fail 2/2 등가 그대로). **⚠️ G3 census 오류 적발 — hook 3종 미삭제**: StepHook(eval-LL live)/PhaseHook(swap dispatch live)/LayerBoundaryHook(LISWAP-4, INV-147 검증 중) 는 orphan 이 아니라 별개 live 서브시스템 — roadmap·SSOT "5-hook 흡수" 문언이 census 오류. 삭제 시 eval/weight-swap 컴파일 붕괴 → 미삭제 + pipeline.rs doc 정오 기재. INV-147/149/150 도 같은 이유로 무접촉(live 불변식). eval/batch orphan(run_eval_ll/run_prompt_batch — 호출처 0 확인)은 ripple 격리 위해 γ-4 백로그로 이관. **host**: chat 20/20·inv_decode 11/11·inv_layer_006/007 GREEN·beta3 10/10·beta4 6/6·beta5 6/6·lib 1315-16 PASS(opencl 외 실패 0)·`session::traits` 잔여 = doc 주석 6건뿐·fmt/clippy clean. **device(S25) 최종**: sig **15/15 MATCH**(순수 삭제 tripwire 무발동) + avg_tbt n=5 median **f16 54.09(Δ−0.24%)/f32 53.97(Δ−0.13%)/q4 53.35(Δ−0.82%)** — **α-K frozen 절대 기준 누적 Δ 전부 음수('진짜 최종 perf' — β 7-substep 누적 오버헤드 0)** + β-3 eviction baseline MATCH(removed=459, sig c41930a5) + resilience-on smoke(Throttle 87.08 적용) + **KIVI 오라클 Q2/Q4/Q8 3/3 PASS(L2=0.000000)**. **test_backend MatMulTransposed 16/MatMulSlice 8 FAIL + RoPE 2 ERROR 는 pre-β(`6e52ac50`) worktree 빌드로 동일 패턴 재현 — 사전존재 하네스 이슈 확정(β 무관; production matmul 정상은 sig 가 증명). 백로그 등록 대상.**
> **★ Phase β 전 substep 완료 (β-1~β-7, 2026-06-10).** 잔여 = 후속 트랙(γ, AB-2/4/6 재개, G4 defer 분).
