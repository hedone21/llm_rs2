...
- **Status**: **설계 ✅** (`design_alpha_k_step5_2026_06_04.md`, 워크플로 `wf_79f8158a-015`: census 4 + design 4 + 적대검증 3 + 합성 1, blocking 4건 반영). 구현 미착수. **설계 핵심 결정**: ① **갈래 A(inherent-only)** — micro-trait 기각(fmt 래퍼가 이미 타입별 분리, YAGNI). ② **KVCacheOps = 삭제 대상**(rename 아님; `format/kv_cache_format.rs::KVCacheFormat` 7-method base trait 이 ADR dispatch 표면 — 이미 실현). ③ **6 증분**: `5-A(거취 결정,코드0) → 5-C∥5-D → 5-B(offload device 필수) → 5-E(inherent rewire,additive) → 5-F(legacy+trait 삭제,비가역 device)`. **★roadmap 누락 발견**: OLD-chain 잔여 소비자가 2개(offload/chunked-prefill)가 아니라 **3개 — KiviForward(`kivi_forward.rs:157/188` `forward_into<KiviCache>`) 신규 식별**(→5-C). **★offload device 매체 부재(BL-1)**: argus_cli 가 offload reject + host CpuBackend 가 offload GPU 경로 미발화 → offload bit-identical baseline 매체가 legacy(5-F 폐기대상)뿐 → **argus-bench(offload 지원)를 5-B hard 선결로 격상** 필요. **★verify.py stale(HB-3)**: `verify.py:99` 가 Cargo.toml 에 없는 `generate` bin 빌드 → legacy 폐기 전 재배선/폐기 결정. 비-happy 모드 거취 권고=**A1(session 함수 라이브러리 보존)**(기능 보존, 5-B/C/D 필수 확정). 상세=설계 문서 §1~8 + 부속 `design_alpha_k_step5_appendix_2026_06_04.md`(census/design/adversarial 원본).
- **Sprint**: 설계 done, 구현 진행 중 — **5-A ✅ / 5-C ✅ host(`641dc932`) / 5-D collapse ✅ / 5-B ✅ host+S25 device(`7a22bb63`+`bb8a200f`) / 5-E ✅ host(`cbb5f376`)**. 다음 = **5-F**(legacy+trait 삭제, 비가역 device).
- **★5-A 결정 확정 (2026-06-04 사용자)**: ① 거취=**A1(session 라이브러리 보존)** → 5-B/5-C fmt 이주 필수. ② 착수=**5-C 선착수**(host). ③ offload device 매체=**legacy 를 offload 한정 5-F 부분 제외**(argus-bench 부담 없이 5-B 진행, legacy offload 경로만 한시 보존 후 별도 제거). ④ verify=**argus-bench/eval 재배선**. **★5-D collapse**: `session::prefill::run_chunked_prefill` 호출처 = `legacy/generate.rs:1848` **단독** 확인 → **이주 불요, dies-with-legacy(5-F 흡수)**. (eval_loop.rs:821 동명 함수는 별개, 이미 ①-d EvalCacheKind fmt.) ⇒ 수정 시퀀싱: `5-A ✅ → 5-C ✅ → 5-B ✅(device) → 5-E → 5-F(5-D 흡수)`.
- **★5-B 완료 (2026-06-04, `7a22bb63` host + `bb8a200f` perf)**: `OffloadFormat`(`Mutex<OffloadKVCache>` interior-mut) + `forward_into_offload_fmt`(copy-fork, DrainGuard) + `OffloadForward`/legacy `run_offload`(decode) `LLMRS_OFFLOAD_FMT` 게이트(transient wrap, ①-c 선례). additive + OFF default byte-불변(OLD forward_into_offload 0 deletion). **host 게이트**: clippy/fmt/non-opencl/offload 62 test clean + **decode arm bit-identical+coherent**(legacy run_offload qwen2.5-1.5b-q4_0 --kv-type f16 n=40, ON≡OFF md5 6aae1595, 발화 로그=vacuous 아님). **S25 device 게이트 PASS**: f16 offload n=32 ON≡OFF bit-identical(md5 46e8e0b7) + **avg_tbt Δ +0.58% median(n=6)** ≪+3% + BL-2 Mutex poisoning 무발생. **적대검증(4 lens wf_0f0ff1d8) blocking 1 적발+해소**: B-1 UAF/drop-while-locked(`?` early-return·패닉이 pending drain 건너뜀 → caller try_unwrap+drop 이 in-flight preload worker raw-ptr 와 경합) → `DrainGuard`(Drop, 모든 반환 경로 drain). W-1 logits overflow(prefill+logits_last_only tail 미처리, OLD heap OOB) → forward_into_fmt:2311 미러 정정. **발산 A 크래시**(BOS-only seq_len=1 prefill, decode arm `.expect`) 수정. **★범위 정밀화**: fmt 게이트 = **decode arm 한정**. forward_into_offload(preload pool) prefill 은 **사전 존재 버그**(빈-캐시 preload→preloaded=true→get_view zero K/V→garbage; OffloadForward chat 은 OLD 부터 broken+비결정) → legacy run_offload 는 prefill=forward_into(정상)라 회피, 게이트도 decode 만. prefill arm = OffloadForward/발산A 전용, causal 정확성은 단위테스트로 격리 검증. **잔여**: Jetson device(unreachable, defer), F32+host-mapped carve-out(W-2, StandardFormat 동일), OLD forward_into_offload + `impl KVCacheOps for OffloadKVCache` 삭제는 **5-F**(5-A: legacy offload 매체 보존).
- **★5-E 완료 (2026-06-04, `cbb5f376`)**: KVCacheOps trait 본문을 KVCache/KiviCache/OffloadKVCache 의 inherent fn 으로 **이전(MOVE)** + trait 메서드는 inherent 위임(`self.method()`, inherent 우선해석으로 recursion 무). fmt 경로·비-forward 소비자·test 의 `KVCacheOps::` 호출 + `use KVCacheOps` import 를 inherent 직호출로 **rewire**. **additive·trait 미삭제**(5-F 가 trait/legacy 삭제). 신규 inherent: KVCache 7(get_view 0-arg→**`view`** rename, 기존 2-arg `get_view(:534)` 동명 충돌 회피=R3) / KiviCache 12(조건부 capacity/layout byte-identical=R2, `is_awqe_enabled`/`total_tokens` 위임, memory_usage_bytes) / OffloadKVCache 8(update/get_view). **★get_view 이름 비대칭**: KVCache 호출처=`.view()`, KiviCache/OffloadKVCache 호출처=`.get_view()`(동명 inherent, 충돌 없음). rewire: standard_format(9)/kivi_format(8)/fmt_bridge(2)/offload_format(4+test4) UFCS→inherent + swap_handler/batch/ppl/kivi_forward import 정리 + offload `compare_views<A,B>` 단형화(KVCache+OffloadKVCache) + test 4종(eng_alg/eng_dat/action_pool/test_backend) import 제거. **host 게이트 전부 clean**: build(opencl+non-opencl+workspace)·fmt·clippy(`--workspace -D warnings`)·**grep §6.2(rewire 대상 KVCacheOps:: 호출 0건)**. lib test 1241 PASS(opencl 21=GPU부재, RSS madvise 2=병렬압박 flakiness 격리PASS, INV-LAYER 2=무관 파일 htp_fastrpc/opencl/plan.rs 사전 baseline staleness). **적대검증(3 fidelity lens, wf_70537a28)**: 3 cache inherent 본문 전부 원본 trait(fd246887)와 **byte-identical**(update/get_view 손 전사 누락/오타 0, `diff` IDENTICAL), 위임 recursion 무(`unconditional_recursion` 0). rewire-neutrality/completeness lens 는 subagent StructuredOutput 미호출+synth 세션한도로 미완 → **cargo 게이트로 대체 검증**(behavior 테스트 PASS + grep0 + clippy clean). **device 불요**(동작 불변). **5-F defer 잔여(trait-only, inherent 미신설)**: KVCache·KiviCache 의 set_current_pos/advance_pos/get_buffers_mut/res_pos/q2_tokens/res_cap/needs_flush/flush_if_needed + OffloadKVCache memory_usage_bytes/set_current_pos — 생존 fmt 소비자 미호출(offload 자체 test + legacy 만 사용) → 5-F 에서 trait 삭제 시 함께 처리. legacy(`:4044/4645`) + offload test 모듈 `use KVCacheOps` 는 이 잔여 호출로 유지.
