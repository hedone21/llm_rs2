...
- **Notes**: B-2 가 full-surface 라 ④-a(getter+advance만 닿는 plan) 로 못 벗긴다(ADR §8.3 B-2). prefill flip 선결이 Step 3((3p)) 의 일부 부담을 미리 던다. **sub-item 분할 가능**:
  - **1a**: B-4 eval flip (독립성 높음, eval_loop + StepHook + CacheSnapshot). host + eval 수치 게이트.
  - **1b**: B-2 prefill flip (forward_prefill + Args fork). host + S25 prefill bit-identical.
  - **1c**: B-2 비-decode forward 잔여 호출처(warmup/qcf_runtime/batch) 단일화. host + 회귀 0.
