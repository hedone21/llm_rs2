# Handoff: Phase 4-3 완료 → Phase 4-4 진입 대기 (+ 디바이스 측정 pending)
...
외부 공개 대비 엔진 레이어드 리팩토링 **Task #4 Phase 4-3 (ModelForward + microbench) 호스트 게이트 완료**. 3 commits, +913 LOC 신규, generate.rs 변경 0. 호스트 CPU Qwen 2.5 1.5B Q4_0 측정에서 **delta_pct=1.53% (PASS), bit-identical=true**. 디바이스 측정 (S25 OpenCL + Jetson CUDA)은 **Tester 위임 대기** (`/deploy-test` + `/profile`). **다음은 Phase 4-4 — main() 조립자화 (≤ 400 LOC, 표준 generate path를 `DecodeLoop+ModelForward`로 교체)**. 단 디바이스 PASS 확인 전까지는 4-4 진입 보류 권장 (회귀 ≥ 5% 시 arch §7.3 escape hatch 필요).
...
| **G6 S25 OpenCL bit-identical** | ⏳ **Tester** |
| **G7 Jetson CUDA bit-identical** | ⏳ **Tester** |
| **G8 avg_tbt Δ ≤ 5% (양 backend)** | ⏳ **Tester** (호스트 CPU Δ=1.53% 참고치) |
...
### 호스트 측정 결과 (papers/eurosys2027/_workspace/experiment/probe_inference_loop_phase4_3_2026_05_17.md)

```
Qwen2.5-1.5b-Q4_0 / CPU / gen=4 / runs=1:
- avg_tbt_ms: decode_loop=62.30 / direct=61.36
- delta_pct: 1.53% (PASS ≤ 5%)
- bit_identical_first_n: true
- tokens: [12095, 13, 576, 6722]
- tok0_ms: decode_loop=176.11 / direct=177.88
```

vtable indirect call 비용 호스트 CPU 1.53%. arch §7.2 이론치(0.0003~0.0014%)보다 크지만 게이트 통과. GPU backend에선 forward 비용이 더 무겁기 때문에 비율이 더 작아질 가능성 (디바이스 측정으로 확정).
...
### 1순위: Phase 4-3 디바이스 측정 (Tester)

**위임 대상**: Tester (`/deploy-test` + `/profile`)
**모델**: opus
**산출**: `papers/eurosys2027/_workspace/experiment/probe_inference_loop_device_2026_05_xx.md`
...
**PASS 게이트**: 두 디바이스 모두 `delta_pct ≤ 5%` AND `bit_identical_first_n=true`.

**FAIL 시**: arch §7.3 escape hatch — `DecodeLoop<F: Forward, T: TokenSampler>` 2-trait monomorphize PoC + 재측정.

### 2순위: Phase 4-4 진입 (디바이스 PASS 확인 후)
