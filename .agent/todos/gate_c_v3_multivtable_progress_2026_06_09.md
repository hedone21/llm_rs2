...
- [x] **V5** `ffea6dec` 게이트: `gate_c_plugin_bundle.rs`(신규, 단일 #[test] 11요소) — v1 게이트 2종을 v2 의미론으로 통합 재작성. example-bundle 에 bundle_perhead(PerHead) 추가 + example-rollback(신규) vehicle. obsolete gate_c_dlopen_equivalence.rs/gate_c_format_dlopen_equivalence.rs 삭제(v1 ABI 제거). **완료 6게이트 전부 green**.

## ✅ 완료 (2026-06-09) — 완료 6게이트
1. `cargo test -p technique-api` 19/0(OFF)·20/0(plugin-cdylib)
2. `cargo test -p llm_rs2 --lib -- --skip backend::opencl --skip memory::opencl` 1260/0
3. `cargo test -p llm_rs2 --test gate_c_plugin_bundle` 1/1(11 단언)
4. `cargo clippy --workspace -- -D warnings` clean
5. `nm` — export 6종 register_kv_formats_v2+register_kv_stages_v2 존재, example_no_export 부재
6. `cargo build --release -p llm_rs2` 54.17s green(fat-LTO)
커밋: V1 `90b04aa7` · V2 `8846d0b3` · V3 `31c8405b` · V4 `62097107` · V5 `ffea6dec` (+ADR `48d77243`, 원장 다수)
...
- (iter 5, 2026-06-09) **V5 완료 = 루프 종료** `ffea6dec`. gate_c_plugin_bundle.rs(11 단언) 통합 재작성 + bundle_perhead + example-rollback + obsolete v1 테스트 2 삭제. **완료 6게이트 전부 green**. ADR-0010 V1~V5 완주. handoff 작성 + notify-send.
