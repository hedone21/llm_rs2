...
## 구현 정정 (host 코드 작성 — 설계 대비 단순화 2건, 2026-06-04)

device-ready 코드를 host 에서 작성하며 설계를 2건 단순화했다 (둘 다 타당, host 게이트 GREEN):

1. **`PlanCacheHandle` trait 미도입 → 직접 `Arc<StandardFormat>` concrete-handle.** production 범위가 Standard 단독(KIVI plan=legacy)이라 generic trait 의 "Standard+KIVI 공유" 이득이 없고(본문은 copy-fork 라 어차피 미공유), `execute_fmt(handles: &[Arc<StandardFormat>])` 가 StandardFormat inherent(`plan_geometry`/`plan_advance`)를 직접 호출하는 게 더 단순(YAGNI — 추측성 trait 회피). trait 은 KIVI plan flip(Step 5-ish) 때 도입. `PlanGeometry` 구조체는 그대로(plan.rs:942).
2. **build_plan buffer seam = `plan_lock()` guard 반환 (설계의 `with_cache` closure 대신).** build_plan 본문이 **장수명 `&KVCache` 슬라이스**를 요구(closure 로는 불가) → `build_plan_fmt` 진입부에서 `Vec<MutexGuard>` 잡고 `Vec<&KVCache>` 재바인딩 → 나머지 본문 byte-identical. guard 는 `build_full_plan`(cl_mem 을 kernel 에 bind) 까지만 유지, FullKernelPlan 은 cache borrow 미보유 → sound. 부수효과: `StandardFormatInner`/`cache` field `pub(crate)`(모듈 내부 한정).

**구현 (전부 additive, `LLMRS_KV_FMT` 게이트 OFF — production-default fmt-OFF byte-불변):**
- `plan.rs`: `PlanGeometry`(:942) + `execute_fmt`(:1945~2605, execute<C> copy-fork). execute<C> **0 deletion**(byte-불변).
- `standard_format.rs`: `plan_geometry()`/`plan_advance()`/`plan_lock()` inherent + unit test 3개. struct/cache `pub(crate)`. 기존 동작 무변.
- `transformer.rs`: `build_plan_fmt`(:2845, build_plan copy-fork+guard preamble) + `execute_plan_fmt`(:3426, execute_plan mirror). build_plan/execute_plan **0 deletion**.
- `model_forward.rs`: `try_build_plan`(fmt active→build_plan_fmt 분기) + `step()` fmt-ON 재배선(plan-on-fmt-handle 먼저→forward_into_fmt 폴백). **fmt-OFF 분기 무변**.

**메인 세션 재검증 (cargo authoritative)**: execute_fmt↔execute / build_plan_fmt↔build_plan / execute_plan_fmt↔execute_plan **기계적 diff = 문서화된 변경뿐**(나머지 byte-identical). 보호 함수 git diff 0 deletion. clippy `--workspace -D warnings` clean(dead_code 0 — rust-analyzer "PlanGeometry never constructed"=stale) + fmt clean + standard_format 16/16 + 전체 lib 1235 pass/23 fail(전부 opencl GPU 부재, 비-회귀). **acceptance(bit-identical+avg_tbt)=device 세션 잔여**(plan GPU-only=host 휴면, fmt-ON 도 host 에선 build_plan_fmt None→forward_into_fmt 폴백).
