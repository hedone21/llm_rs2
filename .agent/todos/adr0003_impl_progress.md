...
> **다음 = M5**(기여자 문서 "기법 추가법" — M2-B·M3·M4·M5 중 마지막 = 루프 완료): docs/ 에 KVCacheStage 기법 추가 가이드 — (1) hook=`KVCacheStage` trait + `StageCtx` 9-accessor, (2) 시그니처/`KVCachePlan`/`KeepSpec`/`WeightedMerge`, (3) 등록=`#[distributed_slice(KV_CACHE_STAGES)]` + **force-link 1줄**(M3 finding), (4) 동작 예제=`crates/techniques/example-keep-recent`, (5) 컴파일·등록 게이트(cargo test + find_stage + release linkme smoke). ADR-0003/0004 + 본 원장 참조. M5 완료 시 전체 sanity + release self-test + 최종 보고 + notify-send → 루프 종료. ⑤(F5 head_importance forward + h2o_plus PerHead + d2o layer-alloc-via-stage·production 전환)는 완료조건 밖(post-session backlog).
> 재개 명령 예: "M5 진행".
