...
> **M4 ✅ 완료**(사용자 결정 A): d2o → `D2OStage`(KVCacheStage). a(9f6c8e5f §4 개정 Q4 보존) / b(fa2819bd apply_weighted_merges≡scatter_reduce F32/F16/Q4) / c-1(17ff35cf dequant_k) / c-2a(72eb3b99 find_nearest reader) / c-2b-i(b8a74266 compute_d2o_plan 공유) / c-2b-ii(f44bdb69 D2OStage + **동등성 통과** d2o_stage_eq F32/F16/Q4 bit-identical) / 655bfaac(d2o 등록 smoke). release smoke green(fat-LTO stage_registry 8/8 + d2o_stage_eq 3/3). production d2o = if-branch(D2OHandler) 유지(non-alloc 한정 등가·layer-alloc 미지원·비권장; D2OStage=proven-equivalent available). test 1234/0.
> **M5 ✅ 완료**(`1d0569a3`): `docs/50_adding_kvcache_stage.md` 기여자 가이드.
>
> ## ✅ 루프 완료 (2026-06-05, iter-17)
> **ADR-0003 + ADR-0004 구현 완수** — M2-B·M3·M4·M5 전부 커밋. 전체 sanity green(test 1234/0, clippy --workspace clean), release self-test green(fat-LTO linkme 생존: stage_registry 8/8 + d2o_stage_eq 3/3). **달성**: stage 축 확장 기법이 정적 link technique crate + linkme 자동 등록으로 추가됨(OCP — 새 기법 = 폴더 + dep 1줄 + force-link 1줄, 기존 로직 수정 0). sliding/streaming/h2o 레지스트리화(World B), per-crate 메커니즘 실증(example-keep-recent), d2o=D2OStage 동등성 bit-identical.
> **Deferred(완료조건 밖, post-session backlog)**: ⑤ F5 head_importance session forward + h2o_plus PerHead executor(현 plan_keep→None, ⑤ 미완) / d2o layer-alloc-via-stage(per-layer budget threading) + d2o production 전환(현 D2OHandler if-branch 유지) / linkme statics 의 `--gc-sections` 장기 모니터링.
> **루프 재개 불필요** — 추가 작업 시 위 deferred 항목 또는 새 지시.
...
- [x] **M4** ✅ d2o → KVCacheStage. M4-a(9f6c8e5f §4 개정 Q4 보존) / M4-b(fa2819bd apply_weighted_merges F32/F16/Q4 ≡ scatter_reduce) / M4-c-1(17ff35cf dequant_k) / M4-c-2a(72eb3b99 find_nearest reader) / M4-c-2b-i(b8a74266 compute_d2o_plan 공유) / M4-c-2b-ii(f44bdb69 D2OStage + **동등성 통과** d2o_stage_eq F32/F16/Q4 bit-identical) / 655bfaac(d2o 등록 smoke). **동등성 PASS**(human-review 불요). release smoke green(fat-LTO: stage_registry 8/8 + d2o_stage_eq 3/3). **production d2o 는 if-branch=D2OHandler 유지**(non-alloc 한정 등가 + layer-alloc 미지원 + 비권장; D2OStage=proven-equivalent available 표면). layer-alloc-via-stage·production 전환 deferred(backlog).
- [x] **M5** ✅ (`1d0569a3`) `docs/50_adding_kvcache_stage.md` — KVCacheStage 기법 추가 기여자 가이드(hook=KVCacheStage+StageCtx, 타입=KVCachePlan/KeepSpec/WeightedMerge, 등록=crate+distributed_slice+force-link, 동작 예제=example-keep-recent, 게이트=cargo test+find_stage+release linkme smoke). API 시그니처 현 코드와 일치 확인. 순수 .md(코드 무변경).
...
- **iter-17 (M5, 2026-06-05) — 루프 완료**: `docs/50_adding_kvcache_stage.md` 기여자 가이드 작성(hook/타입/등록[force-link]/예제/게이트, API 시그니처 현 코드 일치). 순수 .md. 게이트: test 1234/0(첫 run 환경 flake 재실행 green — iter-13 과 동일 패턴, docs 변경이라 회귀 불가), clippy clean, release self-test green(M4). M5 커밋 `1d0569a3`. **M2-B·M3·M4·M5 전부 완료 → 루프 완료 조건 충족**. 최종 보고 + notify-send 후 종료.
- **iter-16 (M4-c-2b-ii, 2026-06-05) — M4 종결**: `D2OStage`(d2o_handler.rs) impl KVCacheStage(plan=compute_d2o_plan(ctx.dequant_k reader)+group_by_retain+compute_eq11_weights→WeightedMerge, EMA=impl Mutex). KVStageCtx+execute_kv_plan pub(crate)+KVStageCtx::new(StageBackedPolicy 도 ::new 사용). D2O_STAGE 등록(non-alloc 기본 D2OConfig). **동등성 테스트 통과**: d2o_stage_eq_handler_{f32,f16,q4} — D2OStage(plan→execute_kv_plan) ≡ D2OHandler(evict_and_merge) buffer **bit-identical**(공유 compute + apply_weighted_merges≡scatter_reduce + 공유 EMA). d2o_stage_registered(find_stage "d2o"). 게이트: build OK, test 1234/0, d2o 54/0, clippy clean, **release smoke green**(fat-LTO stage_registry 8/8 + d2o_stage_eq 3/3). 커밋 f44bdb69 + 655bfaac. → 다음 iter: M5(루프 완료).
