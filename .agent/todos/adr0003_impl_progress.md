...
> **M4-c-1 ✅**(`17ff35cf`): `dequantize_k` pub(crate) + `KVStageCtx.dequant_k` 실구현(위임, bit-identical raw K) + test. test 1230/0.
> **다음 = M4-c-2**(D2OStage + 동등성 — 상세 "## M4-c 구조적 발견 + D2OStage 구현 plan"): D2OStage impl KVCacheStage(plan=evict_and_merge step1~5 재현, mutation 없이, K=ctx.dequant_k, EMA=impl Mutex). bit-identity = find_nearest/evict_and_merge step1~5 를 reader 추상으로 공유 추출(D2OHandler·D2OStage). **동등성 테스트 선작성**: D2OStage(plan→apply_weighted_merges+compact) ≡ D2OHandler(evict_and_merge), **non-alloc config** F32/F16/Q4 bit-identical. **미확립 시 STOP+human-review**. D2OStage 등록(KV_CACHE_STAGES) + d2o if-branch **유지**(production 무변경, layer-alloc·전환 deferred). 이후 ⑤(F5+h2o_plus PerHead), M5.
> 재개 명령 예: "M4-c-2 진행".
