...
> **다음 = M4-c**(d2o KVCacheStage 재구현, M4 마지막): `D2OStage` impl KVCacheStage — `plan(ctx)` = retain_all keep(H2O 3-partition) + group_by_retain·compute_eq11_weights→WeightedMerge, EMA τ=impl `Mutex<D2OState>`, nearest=find_nearest_layer_wide(K는 StageCtx.dequant_k 로 읽기 — **KVStageCtx.dequant_k 실구현 필요**, 현 no-op). **동등성 테스트 선작성 필수**: D2OStage(plan→apply_weighted_merges+compact) ≡ 기존 D2OHandler(evict_and_merge), 동일 call 시퀀스 buffer bit-identical(F32/F16/Q4). apply_weighted_merges≡scatter_reduce(M4-b 증명) + plan==evict_and_merge 내부(헬퍼 재사용) → bit-identical 기대. **미확립 시 STOP + human-review 플래그**. 이후 d2o 등록 + session.rs:604·build_bench_loop.rs:72 if-branch 제거(StageBackedPolicy 경유) 여부. 이후 ⑤(F5+h2o_plus PerHead), M5.
> 재개 명령 예: "M4-c 진행".
