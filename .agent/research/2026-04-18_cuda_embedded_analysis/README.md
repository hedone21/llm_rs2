...
## Phase C1 실험 결과 — H1(sync 배치화) 검증

`--cuda-defer-sync` 플래그 신설. launch 함수 내 암묵적 `synchronize()`를 `maybe_sync()` (플래그 ON 시 no-op)로 교체. 외부 명시적 sync(`read_buffer`, `copy_from`, `cast` F32→F16의 CPU read 경로, `self_test`)는 유지.

### 측정 (3회 반복, 30초 idle 간격)

| 모드 | Decode tok/s | ms/tok | vs baseline |
|---|---:|---:|---:|
| baseline (sync 매번) | 26.3 ± 0.2 | 38.1 | — |
| **defer-sync** | **35.85 ± 0.15** | 27.9 | **+36.3%** |
| llama.cpp (참조) | 35.19 ± 0.2 | 28.4 | +33.8% |

Raw: `raw/c1_defer_run{1,2,3}.log`, `raw/c1_correctness_{baseline,defer}.log`

### Correctness 실패

`--temperature 0.0`로 결정론 출력 비교 (프롬프트 "The capital of France is"):

- baseline: `The capital of France is Paris. It's the most visited city in Europe and one of the world's most popular tourist destinations.` ✓
- defer-sync: `The capital of France islette - – (Re) Jere, Re, Re, Re, Jere-re-re-Re-Jerre-jerre` ✗ (garbage)

### 해석

- **H1 upper bound 확립**: naive sync 제거는 +36% throughput 확보, llama.cpp 초과. CPU/sync overhead의 이론 한계가 12% → 0%에 근접하면 실제 GPU kernel time 33.4ms/tok이 dominate해야 함 (= 30 tok/s). 실측 36 tok/s는 profiler/synchronize 자체가 GPU 유휴시간을 만들었다는 뜻 — launch pipeline을 이어붙이면 kernel끼리도 일부 overlap 가능.
- **정확성 깨짐의 추정 원인**: Jetson UMA 캐시 비일관성. `cuMemHostAlloc` 영역에서 GPU 쓰기가 CPU 캐시에 반영되려면 `cuCtxSynchronize` 필요. decode 중 어딘가 CPU가 UMA 버퍼를 `as_ptr()` 경로로 직접 읽는 지점이 있음 (가장 가능성 높은 후보: sampling 전 logits 경로의 non-`read_buffer` 접근, 또는 layer 간 `Tensor::data()` 같은 helper).
- **현실적 기여도**: 정확한 H1 기여분은 upper bound보다 낮을 것. 구체 경로를 식별해 "필요한 sync만 유지"하는 변형이 필요하나 이 단계에서는 **상한만 기록**.

### C1 후속 과제 (C2 이후로 연기)

1. `maybe_sync()` 계측: 어느 launch 함수에서 sync를 복원하면 correctness가 돌아오는지 bisection
