...
| **Ours-NPU** | Ours HTP FastRPC NPU binding (engine/src/backend/htp_fastrpc/) | `microbench_htp_*` — RMS_NORM 1개만 dispatch GREEN |
...
| 12 | **SOFT_MAX** / SOFTMAX | B | `[nh=12, 1, nctx]` (FlashAttn fused 시 미사용) | ✓ | ✓ | △ 50% | ✗ |
...
→ **Ours-NPU**는 5/25 op (RMS_NORM + MUL_MAT + ADD + MUL + SILU) GREEN. 20/25 op은 init_*_req helper 미구현 = paper "self-built NPU binding scope" evidence.
...
5. **Ours-NPU**는 RMS_NORM + MUL_MAT + ADD + MUL + SILU 5/25 op proof-of-concept (G2 sprint 2026-05-26 — `init_matmul_req` IDL + microbench + S25 deploy, q4x4x2 weight repack 포함; G1 unary sprint 2026-05-27 — `init_binary_req` + `init_unary_act_req` IDL + 3 microbench n_bufs=2/3 dispatch). 20/25 op은 init_*_req helper 미구현 → 본 sprint "self-built NPU binding scope" evidence.
...
| 17 | **GET_ROWS**            | `embed_table[151936,1536]` lookup 1                 | 1           | A | TBD | TBD | TBD | ✗ |
...
| 12 | SOFTMAX                 | `[12, 1, N]` N=ctx (FlashAttn fused 시 미사용)       | 28*         | B | TBD | TBD | TBD | ✗ |
...
"지원 (✓)" cell만 latency 채워지면 evidence 충분. NPU 컬럼은 본 sprint 시점 1 cell (RMS_NORM)만 ✓.
...
### Ours HTP NPU
- 진입: `engine/src/backend/htp_fastrpc/` + `microbench_htp_rmsnorm` (RMS_NORM)
- 측정: 본 sprint 시점 RMS_NORM 1개 dispatch GREEN, 24개는 init_*_req helper 미구현
