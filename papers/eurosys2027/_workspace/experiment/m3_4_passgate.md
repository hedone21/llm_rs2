...
## D-D 진행 결과 (2026-05-10 후속, HEAD 본 commit)

D-D 채택 후 architectural fix 진행:

### D-D.1+D-D.2 (이전 커밋 `3539bd7`, 5/10)

- `crates/qnn_oppkg/src/ops/rope.rs`: `start_pos` SCALAR → input tensor `pos_buf : INT_32 [1]`
- `crates/qnn_oppkg/src/ops/kv_scatter.rs`: `write_pos` SCALAR → input tensor `pos_buf : INT_32 [1]`
- `engine/kernels/simple_ops.cl`: `kernel_rope_simple_oop`/`kernel_kv_scatter_f32_to_f16_oop` 신규 (OOP variant, OpenCL backend 호환 위해 원본 보존)
- standalone microbench (rope/kv_scatter) **9/9 GREEN, pos {0, 10, 100, 1000} max_abs_err = 0**

### D-D.3 (본 commit) — layer_graph 통합

- `engine/src/backend/qnn_oppkg/layer_graph.rs`:
  - `pos_buf` rpcmem slot (idx 23, 4 bytes INT_32) 신규 alloc
  - `t_pos` graph tensor (rank 1, dims [1], INT_32, APP_WRITE) 등록
  - RoPE Q/K nodes: 1 input → 2 inputs (`x_in`, `pos_buf`), params 2 → 1 (theta only)
  - KvScatter node: 2 inputs → 3 inputs (`k_src`, `v_src`, `pos_buf`), params 3 → 2 (head_dim + capacity, write_pos 제거)
  - `AndroidLayerGraph.pos_host_ptr: *mut i32` 신규 필드
  - `execute()` 진입 시 `*(pos_host_ptr) = pos as i32` 1줄 갱신 후 graphExecute
- `engine/src/bin/generate.rs`:
  - qnn_oppkg primary일 때 `gpu_backend_arc`을 OpenCL secondary로 wire (이전엔 qnn_dyn으로 잘못 wire)
  - noshuffle prep을 `prep_backend = gpu_backend_arc` (secondary OpenCL)로 분기

### 디바이스 측정 (Galaxy S25, qnn_oppkg)

```
[qnn_oppkg] eager prebuild: 28 layers, total finalize 1397 ms
[Backend] Noshuffle preparation skipped: Weight buffer has no cl_mem
...
Generating (Max: 2048, Temp: 0, TopP: 0.9, TopK: 40)...
Segmentation fault
```
...
| 측정 항목 | 결과 |
|---|---|
| graphFinalize 28× total | **1397 ms** (RED 시점 1360 ms와 동일) |
| Layer 0 cold | 1200 ms |
| Layer 1~27 warm | 7~8 ms (avg 7.7 ms) |
| 32-token decode 정확성 | **측정 불가** (prefill segfault) |
| TBT / VmRSS | **측정 불가** |

### Root cause: model load weight allocation path mismatch

D-D.3 변경은 multi-pos correctness 측면에선 standalone GREEN 입증. 그러나 production segfault는 layer_graph 자체가 아닌 **model load weight allocation path** 문제:

- qnn_oppkg primary는 `Backend::is_gpu()=true` 반환. transformer model load의 `migrate_to_gpu(qnn_oppkg)`가 weight tensor를 qnn_oppkg backend의 buffer로 이동
- 그러나 qnn_oppkg backend의 `copy_from`은 source buffer를 그대로 clone (passthrough)이므로 실제로는 OpenCL secondary memory로 alloc된 `clMemBuffer`가 weight buffer로 보존됨 — 그렇게 보임
- 그러나 noshuffle prep에서 `Weight buffer has no cl_mem` 메시지 → weight buffer가 OpenCL cl_mem이 아님
- prefill 진입 시 OpenCL secondary fallback이 weight tensor 받아 matmul 수행하지만, weight buffer가 OpenCL이 아닌 buffer type이라 cl_mem dereference에서 segv

이는 **model load 시 weight allocation path 자체를 OpenCL secondary로 강제**해야 해결되는 문제. transformer.rs::TransformerWeights load + migrate logic 변경이 필요한데 본 단계 timebox 초과.

### M3 verdict: **RED 유지**

D-D.3 layer_graph 통합은 spec적으로 완료. multi-pos correctness는 standalone에서 입증. production wire-up은 weight allocation path 문제로 정확성/TBT 측정 불가.

**다음 옵션**:

| 옵션 | 설명 | 추정 |
|---|---|---|
| **D-D.5** | model load 시 weight를 OpenCL secondary memory로 강제 alloc → qnn_oppkg는 host pointer만 graph build 시 SOA 변환에 사용 | 2-3일 |
| **D-E** | M3 scope 재정의 — qnn_oppkg primary 단독 (OpenCL secondary fallback 비활성) + decode-only single-token 측정으로 한정 | 1-2일 |
| **D-F** | qnn_oppkg `copy_from` 갱신 — source buffer가 OpenCL이면 그대로 보존, 아니면 OpenCL로 promote (현 logic 검증 + 강화) | 1-2일 |

본 결정은 plan §M3.4 RED 분기 ("사용자 호출 (scope 재정의)")의 후속. 사용자 결정 필요.
