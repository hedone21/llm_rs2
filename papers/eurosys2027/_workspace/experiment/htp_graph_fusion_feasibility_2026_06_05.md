...
## PoC 결과 (2026-06-05, device 확정) ✅

권장 PoC(`microbench_htp_batch_dispatch`, mm_qkv Q4_0 GEMV N회: sync N×write→read vs async N×write→drain) S25 실측 — **가설 확증**:

| N | A sync (us) | B batch (us) | ratio B/A | err |
|---|---|---|---|---|
| 1 (control) | 146.1 | 145.7 | 1.00 | 2.2e-2 |
| 7 (matmul/layer) | 1032.1 | 449.7 | **0.44** | 2.2e-2 |
| 14 | 2033.9 | 833.8 | **0.41** | 2.2e-2 |
| 28 | 4010.1 | 1424.7 | **0.36** | 2.2e-2 |

- **per-op 비용 146µs(sync) → 47µs(batch)** = 회수된 ~100µs ≈ 예측 floor 정확히 일치.
- N 클수록 ratio 개선(0.44→0.36) = floor amortize 가 더 효과적.
- 전부 max_abs_err 2.2e-2 (single-op 과 동일) = **정확성 보존**.
- N=1 control ratio 1.00 = sanity 통과.

**결론: "async queue batching 으로 dispatch floor 회수" 가설 device-확정.** floor recovery 메커니즘이 실재함을 검증 → layer fusion 의 핵심 전제 성립. 다음: backend `matmul_transposed_via_htp` enqueue/flush 분리 + KV-free 블록(FFN) 부터 layer-loop 통합. 산출물: `htp_batch_dispatch_poc_2026_06_05/`.
