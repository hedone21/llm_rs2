...
| **KVCacheOps** | KV 캐시 추상화 trait (OCP 확장점). `update`, `get_view`, `kv_dtype` 등 | `src/core/kv_cache.rs` |
| **KVCache** | 표준 KV 캐시 (F32/F16/Q4_0). Eviction 지원. KVCacheOps 구현 | `src/core/kv_cache.rs` |
| **KiviCache** | KIVI Q2 압축 캐시. FP32 Residual 버퍼 + Q2 저장소. Eviction 미사용 | `src/core/kivi_cache.rs` |
