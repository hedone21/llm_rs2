...
| **V-12** | `core/events.rs:7` | `crate::core::pressure::{ActionResult, PressureLevel}` | Cross-cutting(observability) → L3 (Pressure concrete) | events.rs는 L3 변경 사항을 표현해야 하므로 의존 자체는 허용. 단 events가 L3 trait의 출력 채널이 되도록 EventSink trait을 통한 inversion 강화. **(γ-1 census, 2026-06-10)**: 현 master에서 `events` 모듈은 통합/제거되어 부재(baseline.json 7건 미포함, `CacheEvent`/`EventSink` 정의 grep 0건) — V-12는 dead. `ActionResult` §13.8-G L2 격상(`engine/src/action_result.rs`) 후에도 V-12 잔존 영향 없음. |
...
| **V-31 (events trait inversion)** | RESOLVED 2026-05-27 (events sprint, HEAD TBD) | 본질: cross-domain sink trait 인프라 (`EventSink` trait + `CacheEvent` super-enum 6 variant + `WeightSwapEvent` 8 sub-variant + `WeightSwapKind` 5 + `NoOpSink`/`StderrDiagnosticSink`/`CollectingSink` 3 impl) 자체를 제거. 53 emit 사이트(production code) → `log::info!`/`log::warn!` macro 직접 호출 + 응축 helper `crate::action_diag_helper::log_swap_*` 8개 + `log_score_diag` 1개 (action_diag_helper.rs L2 top-level 신규, runtime_resources_access.rs precedent). 7 DI 사이트(cache_manager / swap_executor / async_swap / intra_forward_swap / phase_aware_swap / swap_runtime / swap_dispatch) `Arc<dyn EventSink>` field/ctor 인자 전부 제거. `observability/events.rs` 파일 삭제. `WeightSwapKind` enum 제거 (swap_executor.kind: `&'static str`로 단순화). baseline 6→3 (V-31 observability::events 2건 + 미등록 swap_executor/async_swap 2건 모두 해소; V-31 op_trace 1건만 잔존 — PhaseHook L2 격상 별 sprint). 사용자 사전 리뷰: sink는 디버깅 fire-and-forget이라 비즈니스 영향 0 확인 → typed hook 확장(h-1)은 별 sprint, 본 sprint는 외과적 cleanup 한정. | V-31 op_trace 1건 잔존 (PhaseHook L2 격상 backlog) |
...
- **§G 적용 register** (2026-06-10 갱신, 8건 RESOLVED + 1건 PENDING; LayerBoundaryHook 2026-05-26 격상 완료, ActionResult γ-1 L3 도메인 재정의로 추가 — Implementer 작업 대기):
...
| **`ActionResult` (enum + `is_action()` inherent impl)** | **γ-1 L3 도메인 재정의** | **(PENDING — Implementer)** | **`engine/src/kv.rs` (구 `pressure/mod.rs`) → `engine/src/action_result.rs` (L2 직속)** | **L3-kv (`kv/{eviction_handler,d2o_handler,swap_handler,quantize_handler,cache_manager}.rs` + `CachePressurePipeline`) + L3-weight (`weight/weight_swap_handler.rs` `WeightSwapped` variant)** | **γ-1 `pressure/` → `kv/`+`weight/` 분리로 `ActionResult`가 두 도메인 공유 어휘가 됨. `weight_swap_handler.rs`가 `weight/`로 이동하면 `WeightSwapped` 생성을 위해 `kv::ActionResult`를 import → cross-L3 위반. `ActionResult`는 enum-as-data identifier(§13.8-F)이자 kv↔weight 동등 사용(§13.8-G 조건 2 충족)이므로 L2 격상이 정답 — owner를 kv에 두면 weight가 §F를 매번 적용해야 하고, weight에 두면 다수 kv 핸들러가 §F를 적용해야 함. L2 격상으로 import 방향 자체를 소거한다. `CachePressureHandler` trait의 `handle() -> Result<ActionResult>` 반환 타입도 L2 enum을 참조(trait은 kv/ 잔류 — cross-domain trait import는 INV-LAYER-003 허용). `is_action()` inherent method 동반 격상(RAII/lifecycle 아님, 순수 data 판정). `scripts/layer_lint.py` `TOP_LEVEL_L2` set에 `"action_result.rs"` 등재 필요. **선례 정합**: `ModelConfig`(INV-LAYER-003 보조 위계 #3, 양 도메인 공유 configuration L2 격상)와 동일 위계 — 단 ActionResult는 configuration이 아닌 result data identifier. **V-12 정합**: 구 V-12(`observability/events.rs` → `pressure::{ActionResult, PressureLevel}`)는 현 코드에서 events 모듈 통합/제거로 소멸(baseline 7건 미포함, events.rs 부재). ActionResult L2 격상 후 잔존 V-12 영향 없음. PressureLevel(`= llm_shared::Level` type alias)은 kv 도메인 잔류(공유 type alias, 이동 불요).** |
...
- **근거**: cross-L3 trait inversion이 본질 해소이나, 다음 사유로 marker 우선 적용이 cost-effective:
  ...
  - **Weight swap orchestrator**: pressure가 models의 SwapExecutor + LayerSlot + SecondaryMmap을 직접 사용하는 패턴은 WeightSwapDispatch trait + handler 이동(`models/weights/swap_handler.rs`)으로 해소 가능하나, ActionResult enum이 pressure 도메인이므로 잔여 위반 1건 남음 (§F enum-as-data). 격상 ROI는 trade-off. **(γ-1 갱신, 2026-06-10)**: `pressure/` → `kv/`+`weight/` 분리로 `ActionResult`가 kv↔weight 양 도메인 공유 어휘가 되어, §F(단방향 owner 단일 도메인) 전제가 깨짐 → **§13.8-G L2 격상으로 처리**(`engine/src/action_result.rs`, register 참조). weight_swap_handler를 `weight/`로 이동 시 ActionResult import는 L2 직접 경로가 되어 잔여 위반 0건.
...
- **버린 옵션**:
  ...
  - (b) **WeightSwapHandler를 models/weights로 이동**: ActionResult enum 의존 잔존 + pressure/mod.rs re-export ripple. 의미는 있지만 본(S-C3) sprint scope 밖. **(γ-1 후속, 2026-06-10 채택)**: `pressure/` → `kv/`+`weight/` 분리로 `WeightSwapHandler`를 `weight/`로 이동(models/weights/가 아닌 weight/ 도메인). ActionResult는 L2 격상(§13.8-G)으로 enum 의존 잔존 문제 소거. re-export ripple은 `kv.rs`의 `pub use weight_swap_handler::WeightSwapHandler` 1줄 제거 + 테스트 2건 import 경로 갱신으로 한정(production 소비자 0).
...
- **§O 적용 register** (S-C3 sprint 2026-05-24, RESOLVED; Sprint C 2026-05-26 갱신):
  ...
  - **Transformer ctor 위계 어긋남** — **ctor 호출 RESOLVED (Sprint D, 2026-05-26) + field 정의 RESOLVED (Sprint E, 2026-05-27)**:
    - **ctor 호출 해소 결과 (Sprint D)**: `pressure::weights::setup_runtime_resources(backend) -> RuntimeResources` setup helper(`engine/src/pressure/weights/setup.rs` 신설) 도입. `models/loader/mod.rs` production ctor 1건 + `models/transformer.rs` test ctor 5건 모두 setup helper 사용으로 변환. inference 도메인에서 `QuantNoiseTable::empty()`/`PrimaryReleaseWorker::spawn(...)` 직접 호출 0건 달성. 옛 `compute_quant_noise_for_model` 함수(transformer.rs 내) → `pressure::weights::compute_quant_noise(slots, secondary)` 이전(noise_table.rs). marker line: loader/mod.rs 2건 + transformer.rs helper 2건 = **4건 제거**.
    - **field 정의 해소 결과 (Sprint E, 2026-05-27)**: `engine/src/runtime_resources_access.rs` cross-cutting top-level L2 trait file 신설 — `QuantNoiseAccess` + `ReleaseWorkerAccess` trait 정의 + `DrainError`를 trait file로 이전. `TransformerModel::quant_noise` / `release_worker` field type을 `Arc<dyn QuantNoiseAccess>` / `Arc<dyn ReleaseWorkerAccess>` trait object로 격상. struct 정의에서 pressure 타입(`QuantNoiseTable` / `PrimaryReleaseWorker`) 직접 노출 0건 달성. transformer.rs L134/L146 LAYER-EXEMPT marker 2건 제거. 호출지 일괄 마이그레이션 (15+ 사이트): `WeightSwapDecider::noise` / `compute_qcf_weight_swap` / `QcfHelpers::noise_table` / `SwapExecutor::release_worker` / `EngineSwapRuntime::release_worker` / `AsyncSwapDispatcher::release_worker` / `IntraForwardSwapHook::release_worker` 등 — 모두 `&dyn QuantNoiseAccess` 또는 `Arc<dyn ReleaseWorkerAccess>`로 sig 통일.
    - **신규 marker (Sprint E 부작용)**: `runtime_resources_access.rs:16` `use crate::models::weights::LayerWeights;` — cross-cutting top-level L2 trait file의 signature payload (kv_cache_ops가 Tensor를 signature에 노출하는 패턴과 동일 위계). baseline V-34로 명시 (INV-LAYER-002, 의도된 cross-cutting trait payload pattern).
    ...
    - **분류**: 위계 정합 방향 (inference가 pressure trait/helper를 임차). §13.8-O 우선순위 #1 — register 잔존이 정합 상태. **Sprint E 시점**: cross_l3_vocabulary marker 2 제거 + L2→L3 payload marker 1 신규 = 순감 1건. cross-L3 vocabulary 0건 달성.
- **운용 메모**:
...
**§Q — γ-1 L3 도메인 분리: lint 라벨 컨벤션 확정 (2026-06-10, L3 도메인 재정의)**
- **배경**: γ-1(commit `7fe1fe8b`)이 `pressure/`를 `kv/`+`weight/` 두 디렉토리로 물리 분리했으나, `scripts/layer_lint.py`의 `LAYER_RULES`(:81-82)는 `("kv", "L3-pressure")` + `("weight", "L3-pressure")`로 **둘 다 같은 라벨 `L3-pressure`**를 부여한다. 이 상태에서는 `kv↔weight` cross-import가 *동일 도메인 내부 import*로 오판되어 INV-LAYER-003 검사에서 누락된다 (현 시점 weight_swap_handler가 아직 kv/에 있어 위반 0이라 무해하나, handler를 weight/로 이동하면 라벨이 같은 한 위반이 탐지되지 않는다 → false negative).
- **결정 (명세)**: `L3-pressure` 단일 라벨을 **`L3-kv` + `L3-weight` 두 라벨로 분리**한다. SYS-101 4-도메인 {kv, weight, inference, qcf}와 lint 라벨 공간을 1:1 정합시킨다.
  - `LAYER_RULES`: `("kv", "L3-kv")`, `("weight", "L3-weight")`.
  - `_L3_DOMAINS` 튜플: `("L3-pressure", "L3-inference", "L3-qcf")` → `("L3-kv", "L3-weight", "L3-inference", "L3-qcf")`.
  - KNOWN_V_MAP 주석의 "L3-pressure" 라벨 표현 ripple 정정: V-12/V-13(b)/V-24/V-25(e/f) 등 주석의 "pressure→" / "L3-pressure→" 표현을 `kv→`/`weight→`로 갱신(주석이므로 동작 무영향, 정직성 목적).
  - `classify_import` docstring 예시(`"core::pressure::..." → "L3-pressure"`)도 `kv`/`weight` 예시로 갱신.
- **검증 정합**: 라벨 분리 후 `kv→weight` 또는 `weight→kv` concrete import는 INV-LAYER-003 위반으로 정상 탐지된다. `ActionResult` L2 격상 + handler `weight/` 이동이 동반되면 실제 위반은 0으로 유지(L2 경로 import는 위반 아님). 즉 라벨 분리는 **현 위반 수를 늘리지 않으면서**(handler 이동 + L2 격상이 동시 적용되므로) 향후 false negative를 차단한다.
- **버린 옵션**: (a) 라벨 통합 유지(`L3-pressure` 하나로 kv+weight) — kv↔weight 경계가 lint로 강제되지 않아 SYS-101 도메인 분리가 명목상에 그침. γ-1 분리의 검증 가치 상실. (b) `weight/`를 `kv/`의 sub-domain으로 두기 — 자원 owner가 별개(KV resource vs weight resource)이고 cross-import 0이 목표인 수평 독립 도메인이므로 부적합.
