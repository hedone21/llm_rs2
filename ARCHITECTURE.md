...
| **V-?? (hybrid_attention)** | TBD (B-5b sprint Phase 3) | C2 hybrid_attention 2건 (opencl/plan.rs:1546,1552) 해소 — `compute_kv_split`/`current` 호출 위치에 §13.8-J `// LAYER-EXEMPT: dispatch_orchestrator` zone marker 부착. 호출이 build-time read-only이므로 §J 기준 통과. §13.8-J 본문에 "L1 build-time 정책 query 한정, runtime hot path 호출 금지" 보강 1줄. baseline -2. | 없음 |
