...
| **KVCache** | 표준 KV 캐시 (F32/F16/Q4_0). Eviction 지원. KVCacheOps 구현 | `engine/src/core/kv_cache.rs` |
| **KiviCache** | KIVI Q2 압축 캐시. FP32 Residual 버퍼 + Q2 저장소. Eviction 미사용 | `engine/src/core/kivi_cache.rs` |
...
| **LlamaLayer** | 단일 트랜스포머 레이어 (`forward` 내부에서 seq_len에 따라 분기) | `engine/src/layers/llama_layer.rs` |
| **LayerWorkspace** | 생성 루프용 사전 할당 작업 텐서 (매 토큰 재사용) | `engine/src/layers/workspace.rs` |
| **LlamaModel** | 모델 로딩, 임베딩, 레이어 반복, 로짓 계산 | `engine/src/models/llama/llama_model.rs` |
| **AttentionScoreAccumulator** | H2O용 attention importance score 누적 (decay, reset) | `engine/src/core/attention_scores.rs` |
...
### KIVI Q2 Compression (KiviCache 전용)

KIVI 논문(ICML 2024)의 비대칭 2-bit 양자화를 동적 디코드 중에 적용합니다. **Eviction과 상호 배제** — KIVI 모드에서는 CacheManager를 사용하지 않습니다.

**핵심 메커니즘**:
- **Residual Buffer**: 최근 R 토큰(기본 32)을 FP32로 유지. 가득 차면 Q2로 배치 양자화 후 flush.
- **Key: per-channel 양자화** — 각 head의 head_dim 채널별로 R개 토큰 값을 하나의 Q2 블록으로 양자화.
- **Value: per-token 양자화** — 각 토큰의 head_dim 값을 QK2_0(32) 단위로 양자화.
- **BlockQ2_0**: 12 bytes / 32 elements (0.375 bytes/elem). Q4_0 대비 33% 추가 압축.

```
KiviCache 데이터 흐름:
  Token 1~32 → Residual (FP32) → [가득 참] → Q2 Flush → Q2 Storage
  Token 33   → Residual (FP32)
  ...
  get_view() → Q2 Dequantize + Residual Copy → F32 Tensor (기존 attention 코드 호환)
```

`kv_dtype()` 반환값: `F32` → LlamaLayer가 F32 데이터를 `update()`에 전달. KIVI 내부에서 Q2 양자화 처리. `get_view()`는 dequantize된 F32 Tensor 반환 → 기존 attention 코드 변경 없음.
...
│       │   ├── quant.rs               # BlockQ4_0, BlockQ2_0 quantization 구조체
...
```
--kivi                           KIVI Q2 압축 모드 활성화 (eviction과 상호 배제)
...
- **지원 DType**: F32, F16, BF16 (가중치 로딩), Q4_0/Q4_1/Q8_0/Q2_0 (양자화), U8
...
`LlamaLayer`의 `forward()`와 `forward_into()`는 `C: KVCacheOps` 제네릭으로 정의됩니다. 기본 타입 파라미터 `C = KVCache`로 기존 코드 호환성을 유지하며, `KiviCache` 등 새 캐시 구현도 동일 경로를 사용합니다.
