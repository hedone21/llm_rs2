...
impl CudaMemory {
    ...
    /// Compatibility shim — embedded UMA targets always use pinned host memory,
    /// so this is identical to `new()`. Kept to match the PC backend's API
    /// surface so external code (e.g. `bin/generate.rs`) compiles unchanged.
    pub fn managed() -> Self {
        Self
    }
}
