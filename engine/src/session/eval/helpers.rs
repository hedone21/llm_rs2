...
pub fn load_eval_questions(
    args: &Args,
    default_prompt: &str,
) -> anyhow::Result<Vec<crate::eval::EvalQuestion>> {
    ...
    let mut questions: Vec<crate::eval::EvalQuestion> = Vec::new();
    for task in &raw_tasks {
        if let Some(choices) = task["choices"].as_array() {
            questions.push(crate::eval::EvalQuestion {
...
pub fn build_eval_ll_warmup_text(
    questions: &[crate::eval::EvalQuestion],
    max_tokens: usize,
