...
// Tiled Q4_0 x F32 GEMM for prefill.
// Ported from llama.cpp mul_mm_q4_0_f32_l4_lm.cl, adapted to llm.rs interleaved
// BlockQ4_0 layout (single buffer, 18 bytes/block: half d + uint8_t qs[16]).
//
// Differences from upstream:
//  - src0_q (uchar4*) + src0_d (half*) separate buffers → single `block_q4_0*`
//  - uchar4 load replaced with byte-level reads (uchar4 requires 4-byte alignment,
//    but qs[iqs*4] within an 18-byte struct may start at an odd offset).
...
kernel void kernel_mul_mm_q4_0_f32_l4_lm(
    ...
    src0 = (global struct block_q4_0 *)((global char*)src0 + offset0);
    ...
    for (int block = 0; block < ne00; block += BK) {
        for (int l = 0; l < BM; l += loadstride_a) {
            if (ir*BM + loadc_a + l < ne01) {
                ...
                global struct block_q4_0 * blk = src0 + ib;
                float d = (float)blk->d;
                global uchar * qbytes = blk->qs + iqs*4;
                uchar4 q = (uchar4)(qbytes[0], qbytes[1], qbytes[2], qbytes[3]);
