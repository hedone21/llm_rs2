...
// Adreno-optimized F16 GEMV: N_DST=4 rows per wave, float4 accumulators,
// pre-computed row pointers for minimal register pressure.
#define N_DST 4
...
kernel void kernel_mul_mat_f16_f32(
    ...
    // Pre-compute row pointers (keep in registers, avoid multiply in loop)
    ...
    // float4 accumulators — good ILP with 4 independent lanes
    float4 acc0 = (float4)(0.f);
    float4 acc1 = (float4)(0.f);
    float4 acc2 = (float4)(0.f);
    float4 acc3 = (float4)(0.f);

    // Vectorized main loop: vload4 + convert_float4 + float4 multiply-add
