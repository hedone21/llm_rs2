...
// Adreno-optimized F16 GEMV — 4-wave K-split, 128 rows/WG
//
// Layout: local_size = [64, 4, 1]  (4 subgroups x 64 lanes)
//   - 4 waves split K dimension: each wave handles K/4
//   - Each fiber handles 2 output rows (N_DST=2)
//   - WG covers 64 * 2 = 128 output rows
//   - Wave-level partial sums reduced via local memory
//
// Dispatch: global = [ceil(n/128)*64, 4, m], local = [64, 4, 1]
#define N_DST 2
...
kernel void kernel_mul_mat_f16_f32(
    ...
    // 128 rows per WG, 2 rows per fiber
    ...
    // chunk_size = ceil(ne00 / 4), but we loop with stride inside
    ...
    // Each fiber strides by 64*4=256 through its K chunk
    ...
    local float2 reduce_lm[N_SIMDWIDTH * 3];  // 64 * 3 float2 = 1536 bytes

    float2 partial = (float2)(sumf0, sumf1);
