...
fn main() -> anyhow::Result<()> {
    ...
    if matches!(args.effective_kv_mode(), KvMode::Kivi) && args.eval_ll {
        ...
        let eval_config = llm_rs2::session::eval::EvalConfig {
            max_seq_len,
            effective_budget: 0,
            kv_budget_ratio: 0.0,
            greedy: args.greedy,
        ...
        let mut hook = llm_rs2::session::eval::KiviHook::new(
            qcf_config,
            args.enable_qcf_experimental,
            kivi_sample_layers,
            kivi_score_acc,
        );
        let output = llm_rs2::session::eval::run_eval_ll_generic(
            &model,
            &tokenizer,
            &backend,
            &*memory,
    ...
    if let Some(ref dump_path) = args.qcf_dump {
        use llm_rs2::session::eval::qcf_helpers::{QcfSwapDumpContext, dump_qcf_swap_json};
