//! M2.F CustomFlashAttn correctness — production OpPackage vs raw OpenCL.
//!
//! Maps to `flash_attn_f32_f16_q1` in `engine/kernels/flash_attn_f32_f16.cl`.
//! The decode kernel performs:
//!   - score = Q · K^T * scale  (online softmax)
//!   - p = exp(score - m_final)
//!   - O = Σ p · V  / l_final
//!
//! Three test cases at (n_head=32, n_head_kv=2, head_dim=128):
//!   - n_kv ∈ {128, 1024, 2048}
//!   - Random F32 Q, F16 K, F16 V
//!   - is_causal=0, scale=1/sqrt(128)≈0.0884
//!   - Reference: raw OpenCL `flash_attn_f32_f16_q1` (same kernel binary)
//!   - Test: QNN OpPackage `CustomFlashAttn`
//!   - Assert: max_abs_err < 1e-2 (online softmax accumulation tolerance)
//!
//! Same-kernel comparison: errors should be near zero. Threshold 1e-2 absorbs
//! any ULP drift from QNN graph-internal buffer copies or arg routing.
//!
//! Build (Android cross):
//!   cargo build --release --features qnn,opencl --target aarch64-linux-android \
//!     --bin microbench_qnn_oppkg_flash_attn_correct
//!
...
//!              /data/local/tmp/microbench_qnn_oppkg_flash_attn_correct"

#[cfg(not(feature = "qnn"))]
fn main() {
    eprintln!("microbench_qnn_oppkg_flash_attn_correct requires --features qnn");
    std::process::exit(2);
}
...
fn main() -> anyhow::Result<()> {
    ...
    println!("=== microbench_qnn_oppkg_flash_attn_correct (M2.F) ===\n");
    ...
    // ── Path A setup: raw OpenCL reference ───────────────────────────────────
    ...
    // ── Path B setup: QNN backend + register OpPackage ───────────────────────
    ...
    for &n_kv in &[128usize, 1024, 2048] {
        println!(
...
/// Convert f32 to f16 bits (round-to-nearest-even for normal numbers, simple).
#[cfg(feature = "qnn")]
...
fn run_case(
    ...
    // K, V: F16 in [-0.5, 0.5], only positions [0..n_kv) populated. The rest
    // can be garbage — the kernel respects `n_kv` as the loop bound.
    ...
    // Mask buffer for OpenCL ref: production binds `mem_null()` for mask and
    // the kernel skips the mask path. We pass a dummy 1-elem buffer here too
    // and set the `mask_void` arg to `mem_null` directly.
    ...
    // Strides (matches descriptor build_layout)
    ...
    // Reference path uses literal null for mask/sinks (production behaviour).
    ...
    let rpc_sinks = unsafe { rpcmem_alloc(heap_id, flags, dummy_bytes) };
    ...
    unsafe {
        ...
        std::ptr::write_bytes(rpc_sinks as *mut u8, 0, dummy_bytes as usize);
    ...
    let g_name = CString::new(format!("fa_graph_{}", n_kv)).unwrap();
