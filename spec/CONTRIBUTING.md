...
llm_rs2의 설계 문서(`spec/`)가 **불변 요구사항**과 **구현 상세**를 한 파일에 혼재하고 있다.

```
spec/22-manager-algorithms.md 안에:
  [INV-031] integral은 [0, integral_clamp] 범위 내이다.  ← 불변 (구현이 바뀌어도 유지)
  코드 위치: manager/src/pi_controller.rs:87             ← 구현 상세 (리팩토링하면 바뀜)
  config 키: policy.pi.integral_clamp = 2.0              ← 구현 상세
```

이로 인해:
- 코드 리팩토링 시 spec 파일을 건드려야 하고
- spec 변경 시 어디가 불변이고 어디가 구현인지 구분이 안 되고
- 테스트가 "무엇을 검증하는지"(spec) vs "어떻게 검증하는지"(구현)가 섞임

### 해결

**Spec(불변) → Architecture(가변) → Test(검증)** 3계층으로 분리한다.

| 질문 | 계층 | 변경 빈도 |
|------|------|----------|
| 무엇이 보장되어야 하는가? | **spec/** | 낮음 |
| 코드에서 어떻게 실현하는가? | **arch/** | 높음 |
| 실제로 보장되는가? | **tests/spec/** | 중간 |

단일 ID(`INV-031`)가 3계층을 관통하여 **정의 → 구현 → 검증**을 한 번에 추적할 수 있다.

### 이 문서의 목적

이 가이드를 따르면:
1. spec/을 변경할 때 **무엇을 동시에 갱신해야 하는지** 알 수 있다
2. arch/를 작성할 때 **어떤 형식으로 어디에** 쓸지 알 수 있다
3. 테스트를 추가할 때 **어떤 INV를 어떤 전략으로** 검증할지 알 수 있다
4. 일관성 검사를 **자동화**하여 3계층 동기화를 유지할 수 있다
...
### 3.2 Phase 1: spec/ 내부 normative/non-normative 분리
...
> **참고**: arch/ 디렉토리는 Phase 2에서 생성된다. 이 섹션은 생성 시 따를 규칙을 미리 정의한다.
