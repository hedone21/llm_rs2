...
### 3.28 DecodeLoop v3 PipelineStage Invariants [INV-DECODE-STAGE-001 ~ INV-DECODE-STAGE-007]

2026-05-27 DecodeLoop v3 (단일 `PipelineStage` trait + lifecycle phase enum + entry point별 `PipelineRegistry`) 의 불변식. 대응 명세: `arch/pipeline_stage_design.md` §3, §4, §5, §8.

**도입 컨텍스트**: `arch/inference_pipeline.md` v1 (Phase 4-2 7-trait) 의 잔여 문제 (R-2 EvictionStage 시그니처 부족, R-5 ResilienceStage 신설 결정점, decode_fallback God Ctx 12/21 필드, Manager IPC wiring 격차, 신규 책임 추가 비용) 를 단일 hook 패턴으로 재해결. 23 라운드 grill 결정. Phase α/β/γ 3 단계 8~13주 작업.

**ID 컨벤션**: 본 시리즈는 `INV-DECODE-STAGE-NNN` 별칭 형식. INV-LAYER / INV-RPCMEM 시리즈와 동일하게 INV-NNN 의 numeric range 와 충돌하지 않으며, 의미상 별도 차원 (DecodeLoop v3 hook pattern) 만 다룬다.

**교차 참조**:
- **INV-LAYER-006 / INV-LAYER-007** (DecodeLoop 추상화 결합도 + typestate builder): 본 시리즈는 v3 재설계 후에도 양 INV 정신을 보존한다. `DecodeLoop` 필드는 모두 `Box<dyn>` / `Arc<dyn>` 또는 generic bound (INV-LAYER-006). `DecodeLoopBuilder` typestate는 v2 보존 (INV-LAYER-007).
...
- **INV-147** (intra-forward swap hook=None zero overhead, < 1%): 본 시리즈의 PoC TBT 게이트 (≤ +3%) 는 INV-147 noise 기준의 3배. PipelineStage v0 + EvictionPolicyStage + 6~8 phase iteration overhead 를 INV-147 micro-noise 위에서 측정.
- **INV-121 / INV-123** (per-token snapshot + ArcSwap atomic): Phase β 진입 후 `KvBundle::prune` / `WeightBundle::swap_layer` 등 mutation method 가 INV-121/123 정신을 그대로 보존해야 한다 (impl 책임, INV-KVBUNDLE-SYNC 의 일부).
- **INV-149 / INV-150** (commit-before-read ordering, plan run-to-completion): SwapDispatchStage Persistent stage 가 PostLayer phase 에서 dispatch + WeightBundle::swap_layer 호출 시 동일 정신 보존.

**테스트 위치**: 본 시리즈는 `engine/tests/spec/test_inv_decode_stage_{001..007}.rs` (또는 INV 묶음별 그룹화) 에 spec test 파일을 가져야 한다. INV-DECODE-STAGE-001/002/005/006 (정적 / 코드리뷰 의무) 은 source-grep + PR checklist 로 검증. INV-DECODE-STAGE-003 (SYNC) / 004 (OUTCOME) / 007 (LIFECYCLE) 은 runtime + test 검증.

| ID | 원본 | 한줄 요약 | 카테고리 | 검증 | 비고 |
|----|------|----------|---------|------|------|
| INV-DECODE-STAGE-001 | arch/pipeline_stage_design.md §3.1, §8.1 (KV-PHASE) | `PipelineStage` 구현체는 `ctx.kv` mutation method (`prune` / `offload` / `recall` / `set_layer_dtype` / `merge_evicted` / `compress_prefix` / `set_sparse_pattern`)를 `LifecyclePhase::PreLayer` / `PostLayer` / `Fine(*)` 어느 것에서도 호출하지 않는다. 허용 phase 는 `PreEviction` / `PostEviction` / `PreSwap` / `PostSwapBefore` / `PostSwapAfter` / `PreForward` / `PostForward` / `PreSample` / `PostSample` / `PrefillStart` / `PrefillChunkBoundary` / `PrefillEnd` / `SessionStart` / `SessionEnd` / `DecodeStart` / `DecodeEnd` / `TurnStart` / `TurnEnd` / `Finalize`. 위반 시 step 안 layer 간 KV state inconsistency → logits corruption. 컴파일/런타임 강제 X — stage 구현자가 자기 phase 매칭에서 책임 (M1: spec 경고 + trait doc comment). | Correctness | static (코드리뷰), test | `PipelineStage` trait doc comment 본문 + PR checklist 의무화. 4 PoC test 중 KIVI per-layer mix 가 본 INV 회귀 검출. impl 별 source-grep 시 mutation method 호출이 `LifecyclePhase::{Pre,Post}Layer` 또는 `Fine(*)` 분기 안에서 등장하지 않음을 확인. |
| INV-DECODE-STAGE-002 | arch/pipeline_stage_design.md §3.5 (KvBundle), §8.1 (KVBUNDLE-CONSISTENCY) | `KvBundle` 시그니처는 method 별 layer scope 가 명시되어야 한다. layer-wide: `prune`, `offload`, `recall`, `compress_prefix`. per-layer (`idx` 명시): `set_layer_dtype`, `set_sparse_pattern`. caller-defined scope: `merge_evicted` (evicted/nearest/weights slice 의 scope 는 caller 결정 — layer-wide concat 또는 per-layer 호출). 위반 시 동일 method 호출이 caller 마다 다른 의미를 가져 silent KV state drift 유발. | Correctness | static (코드리뷰) | `kv_bundle.rs` trait method doc comment + 호출자 측 spec test. `KvBundle` impl 추가 시 동일 method 의미 보존 의무. Q24-1 sub-trait round (Pre-α-1) 에서 시그니처 finalize. |
| INV-DECODE-STAGE-003 | arch/pipeline_stage_design.md §3.5 (KvBundle), §8.1 (KVBUNDLE-SYNC) | `KvBundle` mutation method (`prune` / `offload` / `recall` / `set_layer_dtype` / `merge_evicted` / `compress_prefix` / `set_sparse_pattern`) 는 호출 후 GPU work 완료를 impl 내부에서 보장한다 — `backend.synchronize_kv_queue()` 또는 동등 barrier. 위반 시 mutation 직후 발동하는 후속 stage (예: KvMergeStage 의 cosine 계산) 가 미완성 GPU buffer 위에서 동작하여 garbage 산출. INV-149 (LISWAP commit-before-read ordering) 정신의 일반화 — 본 INV 는 KvBundle 단위로 의무 격리. | Safety / Correctness | runtime, test | KvBundle impl 별 method 본문 검사 + 디바이스 e2e 정확성 게이트 (S25 Qwen2.5-1.5B Q4_0 32 토큰 bit-identical). Phase β 진입 시 host stub backend 로 sync 호출 검증. |
...
| INV-DECODE-STAGE-006 | arch/pipeline_stage_design.md §3.3 (god ctx), §8.1 (CTX-AUTHORITY) | `StageContext` 5 field (`step` / `kv` / `weights` / `backend_ext` / `profiler`) mutation 권한 강제 X — code review 책임 (M1만). 특정 stage 가 자기 책임 범위 밖 field 를 mutate 하더라도 컴파일/런타임 검출 없음. PR checklist 의무화 — 신규 stage 추가 시 어느 ctx field 를 어느 phase 에서 mutate 하는지 명시 + reviewer 확인. 위반 시 (예: `OneShotKvQuantStage` 가 `ctx.weights.swap_layer()` 호출) silent 책임 경계 위반 — code review 가 유일한 방어선. | Correctness | static (코드리뷰), test | PR checklist + stage 별 책임 매트릭스 (`arch/pipeline_stage_design.md` §5.3) 와의 정합성 검토. Phase β 진입 시 review template 도입. |
