...
2026-05-27 DecodeLoop v3 (단일 `PipelineStage` trait + lifecycle phase enum + entry point별 `PipelineRegistry`) 의 불변식. **2026-05-28 본 grill 갱신**: KvBundle/WeightBundle trait 폐기 + KVCacheLayer/WeightLayer trait + ctx 5→3 field 결정 반영. **대응 명세: `arch/pipeline_stage_design_v2.md` (2026-05-29 clean 재작성, 현재 상태 단일 진실원본). `arch/pipeline_stage_design.md` (v1) 는 결정 이력 보존용.**
...
- **INV-121 / INV-123** (per-token snapshot + ArcSwap atomic): Phase β 진입 후 `KVCacheFormat::compact` / `WeightFormat::apply_storage` 등 mutation method 가 INV-121/123 정신을 그대로 보존 (impl 책임).
- **INV-149 / INV-150** (commit-before-read ordering, plan run-to-completion): SwapDispatchStage Persistent stage 가 PostLayer phase 에서 dispatch + WeightFormat::apply_storage 호출 시 동일 정신 보존.
