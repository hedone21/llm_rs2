...
2026-05-27 DecodeLoop v3 (단일 `PipelineStage` trait + lifecycle phase enum + entry point별 `PipelineRegistry`) 의 불변식. **2026-05-28 본 grill 갱신**: KvBundle/WeightBundle trait 폐기 + KVCacheLayer/WeightLayer trait + ctx 5→3 field 결정 반영. 대응 명세: `arch/pipeline_stage_design.md` §3, §4, §5, §8.
...
| **INV-KVCACHELAYER-PRIMITIVE-AGNOSTIC** (신규 2026-05-28, 본 sub-grill 2026-05-28~29 갱신) | arch/pipeline_stage_design.md §3.5 (KVCacheLayer), §8.1 | `KVCacheLayer` mutation method (`write_kv` / `write_kv_batch` / `compact`) 는 storage-format-agnostic. Tier 1 primitive-only Stage 가 dtype / codebook / rotation matrix / sparse pattern 알면 안 됨. KVCacheLayer impl (`StandardLayer` / `KIVILayer` / `SparseLayer` 등) 이 mechanism 캡슐화. 새 paradigm 추가 = 새 impl 만, Tier 1 stage 변경 0. **본 sub-grill 2026-05-28~29 결정 #15 갱신**: 적용 범위는 **Tier 1 (primitive-only) Stage 한정**. Tier 2 paradigm-specific Stage (예: `KviQuantizeStage` with `Arc<KIVILayer>`) 는 의도적으로 paradigm 의식 OK. Tier 3 cross-paradigm Stage 는 Stage 측 capability trait 의 추상화 책임. **본 sub-grill 결정 #15 부산물**: `apply_storage(spec)` method **폐기** (storage paradigm 별 mutation 은 Tier 2 가 concrete struct method 직접 호출로 흡수). `KVCacheLayer::as_any()` 부재 (downcast 의도적 차단). **본 sub-grill 결정 #18 정합**: `KVCacheView::dtype()` 부재 — Stage 가 storage paradigm 모름 정신 일관. Backend / Layer 내부 보관. 위반 시 Tier 1 stage 가 storage paradigm 별 분기 로직 가짐 → OCP 위반 + 새 paradigm 추가 cost 폭증. INV-DECODE-STAGE-002 (폐기) 정신의 강화. | Correctness | static (코드리뷰), test | Tier 1 Stage impl source-grep 으로 `match storage_type` / `if dtype == ...` / `as_any().downcast` 등 storage-aware 분기 부재 확인 + paired KVCacheLayer impl 별 test. PR checklist 에 신규 Stage 의 tier 분류 + storage-agnostic 의무 (Tier 1 한정) 명시. |
...
| **INV-STAGE-LAYER-HANDLE** (신규 2026-05-28, 본 sub-grill 2026-05-28~29 갱신) | arch/pipeline_stage_design.md §3.3 / §5.2 (Stage 패턴), §8.1 | `PipelineStage` 는 자기 책임 layer handle 을 stage 객체 register 시점 보관. `StageContext` 에 `kv` / `weights` field 두지 않음. (γ) interior mutability 모델 — Stage 가 `&self` 통해 layer mutation method 호출. 본 프로젝트 `LayerSlot::rcu_weights` (`engine/src/models/weights/slot.rs:158`) 패턴의 자연 확장. **본 sub-grill 2026-05-28~29 결정 #15/#16 갱신** — Stage 의 본질에 따라 3-tier 패턴 적용: (Tier 1) base trait `Arc<dyn KVCacheLayer>` / `Arc<dyn WeightLayer>`, (Tier 2) concrete `Arc<ConcreteLayer>` (예: `Arc<KIVILayer>` — register 시점 compile-time type 강제, downcast 0), (Tier 3) capability trait `Arc<dyn CapabilityTrait>` (Stage 측 정의, base trait 변경 0). **Cardinality 자유 (1/N/0 layer)** — 1 layer (signal-driven paradigm-specific) / N layer (cross-layer policy) / 0 layer (backend-only). 임의 1-layer-per-stage 가정 폐기. 위반 시 ctx god abstraction 재발생 + Stage 가 자기 책임 외 layer 접근 risk. | Correctness | static (코드리뷰), test | Stage struct 정의 source-grep 으로 `kv: &mut dyn ...` / `weights: &mut dyn ...` 등 ctx 통한 layer 접근 부재 확인. Stage 가 layer handle field (`Arc<dyn KVCacheLayer>` / `Arc<dyn WeightLayer>` / `Arc<ConcreteLayer>` / `Arc<dyn CapabilityTrait>`) 보유 패턴 PR checklist + tier 분류 (kv/ vs weight/ vs system/) 의무화. |
