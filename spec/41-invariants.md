...
- **capability 노출 통일**: per-forward `as_kivi_attention()`/`gpu_score_acc()` lookup 폐기 → 소비자가 `CapabilityRegistry` (typed anymap) 로 construction 시점 handle 보유. as_any/as_kivi/gpu_score/get_extension 4 메커니즘 → 1 registry 수렴 (hot path 분기 0).
