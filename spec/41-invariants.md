...
| **INV-KVCACHELAYER-PRIMITIVE-AGNOSTIC** (신규 2026-05-28) | arch/pipeline_stage_design.md §3.5 (KVCacheLayer), §8.1 | `KVCacheLayer` mutation method (`write_kv` / `write_kv_batch` / `compact` / `apply_storage`) 는 storage-format-agnostic. Stage 가 dtype / codebook / rotation matrix / sparse pattern 알면 안 됨. KVCacheLayer impl (`StandardLayer` / `KIVILayer` / `SparseLayer` 등) 이 mechanism 캡슐화. 새 paradigm 추가 = 새 impl 만, stage 변경 0. 위반 시 stage 가 storage paradigm 별 분기 로직 가짐 → OCP 위반 + 새 paradigm 추가 cost 폭증. INV-DECODE-STAGE-002 (폐기) 정신의 강화. | Correctness | static (코드리뷰), test | Stage impl source-grep 으로 `match storage_type` / `if dtype == ...` 등 storage-aware 분기 부재 확인 + paired KVCacheLayer impl 별 test. PR checklist 에 신규 Stage 의 storage-agnostic 의무 명시. |
...
| **INV-STAGE-LAYER-HANDLE** (신규 2026-05-28) | arch/pipeline_stage_design.md §3.3 / §5.2 (Stage 패턴), §8.1 | `PipelineStage` 는 자기 책임 layer handle (`Arc<dyn KVCacheLayer>` / `Arc<dyn WeightLayer>`) 을 stage 객체 register 시점 보관. `StageContext` 에 `kv` / `weights` field 두지 않음. (γ) interior mutability 모델 — Stage 가 `&self` 통해 layer mutation method 호출. 본 프로젝트 `LayerSlot::rcu_weights` (`engine/src/models/weights/slot.rs:158`) 패턴의 자연 확장. 위반 시 ctx god abstraction 재발생 + Stage 가 자기 책임 외 layer 접근 risk. | Correctness | static (코드리뷰), test | Stage struct 정의 source-grep 으로 `kv: &mut dyn ...` / `weights: &mut dyn ...` 등 ctx 통한 layer 접근 부재 확인. Stage 가 `Arc<dyn KVCacheLayer>` / `Arc<dyn WeightLayer>` field 보유 패턴 PR checklist. |
