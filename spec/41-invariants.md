...
- **INV-121 / INV-123** (per-token snapshot + ArcSwap atomic): Phase β 진입 후 `KVCacheLayer::compact` / `WeightLayer::apply_storage` 등 mutation method 가 INV-121/123 정신을 그대로 보존 (impl 책임).
- **INV-149 / INV-150** (commit-before-read ordering, plan run-to-completion): SwapDispatchStage Persistent stage 가 PostLayer phase 에서 dispatch + WeightLayer::apply_storage 호출 시 동일 정신 보존.
...
| **INV-KVCACHELAYER-PRIMITIVE-AGNOSTIC** (신규 2026-05-28, 본 sub-grill 2026-05-28~29 갱신) | arch/pipeline_stage_design.md §3.5 (KVCacheLayer), §8.1 | `KVCacheLayer` mutation method (`write_kv` / `write_kv_batch` / `compact`) 는 storage-format-agnostic. base-trait-handle (`Arc<dyn KVCacheLayer>`) 을 보유한 Stage 가 dtype / codebook / rotation matrix / sparse pattern 알면 안 됨. KVCacheLayer impl (`StandardLayer` / `KIVILayer` / `SparseLayer` 등) 이 mechanism 캡슐화. 새 paradigm 추가 = 새 impl 만, base-trait-handle stage 변경 0. **본 sub-grill 결정 #15 갱신 (2026-05-29 handle-form 정련)**: 적용 범위는 **base trait object handle (`Arc<dyn KVCacheLayer>`) 을 보유한 Stage 한정**. concrete-handle Stage (예: `KviQuantizeStage` with `Arc<KIVILayer>`) 가 그 paradigm 을 아는 것은 위반 아님 (concrete 를 든다는 의미 자체). capability-handle Stage 는 Stage 측 capability trait 의 추상화 책임. 규칙은 tier 라벨이 아니라 handle *타입* 에서 자동 도출. **본 sub-grill 결정 #15 부산물**: `apply_storage(spec)` method **폐기** (storage paradigm 별 mutation 은 Tier 2 가 concrete struct method 직접 호출로 흡수). `KVCacheLayer::as_any()` 부재 (downcast 의도적 차단). **본 sub-grill 결정 #18 정합**: `KVCacheView::dtype()` 부재 — Stage 가 storage paradigm 모름 정신 일관. Backend / Layer 내부 보관. **Q-#1-4/5 해소 (2026-05-29)**: `KVCacheView` + `view()` 자체 **삭제** — read 는 geometry(`idx`/`current_pos`/`capacity`, Layer 본체) + content(concrete-handle read-only inherent method, Q-#1-3 (a)) 로 분리. #18 + Q-#1-3 (a) 이후 view 는 멤버 0·소비자 0(deletion test 불통과). capacity 중복(Q-#1-4) 원천 소거 + mutation 누설(Q-#1-5) 불가(제네릭 read view 부재). 승격 trigger: 2번째 paradigm-agnostic content-read 소비자 시 재도입. 위반 시 Tier 1 stage 가 storage paradigm 별 분기 로직 가짐 → OCP 위반 + 새 paradigm 추가 cost 폭증. INV-DECODE-STAGE-002 (폐기) 정신의 강화. | Correctness | static (코드리뷰), test | base-trait-handle Stage impl source-grep 으로 `match storage_type` / `if dtype == ...` / `as_any().downcast` 등 storage-aware 분기 부재 확인 + paired KVCacheLayer impl 별 test. PR checklist 에 신규 Stage 의 handle 형태 + storage-agnostic 의무 (base-trait-handle 한정) 명시. |
| **INV-KVCACHELAYER-PAIRED-KERNEL** (신규 2026-05-28) | arch/pipeline_stage_design.md §3.5 (KVCacheLayer), §8.1 | KVCacheLayer impl 과 paired backend attention kernel 매핑 의무 — `attention_into` (base trait method, 2026-05-30 ④ grill 추가) 가 impl 별 paired kernel dispatch (NVIDIA fused / Adreno dequant) 를 캡슐화하여 이 매핑을 코드로 실체화한다. 새 storage paradigm = 새 layer impl + paired kernel set (예: KIVILayer + kivi attention kernel + kivi flash attn). Sparse layer + standard attention kernel 등 mismatched 조합 금지. Q9 호환성 차단 인프라 X 결정 (본 grill #5) — 만나면 panic. 위반 시 silent garbage 출력. | Correctness | static (코드리뷰), runtime (panic) | KVCacheLayer impl 별로 paired kernel set 명시 (impl-level doc comment + arch 문서). runtime mismatched 호출 시 panic. PR checklist 에 신규 layer impl 의 paired kernel 명시 의무. |
| **INV-STAGE-LAYER-HANDLE** (신규 2026-05-28, 본 sub-grill 2026-05-28~29 갱신) | arch/pipeline_stage_design.md §3.3 / §5.2 (Stage 패턴), §8.1 | `PipelineStage` 는 자기 책임 layer handle 을 stage 객체 register 시점 보관. `StageContext` 에 `kv` / `weights` field 두지 않음. (γ) interior mutability 모델 — Stage 가 `&self` 통해 layer mutation method 호출. 본 프로젝트 `LayerSlot::rcu_weights` (`engine/src/models/weights/slot.rs:158`) 패턴의 자연 확장. **본 sub-grill 결정 #15/#16 갱신 (2026-05-29 handle-form 정련)** — Stage 가 보유하는 layer-handle 은 Rust 핸들 타입에 대해 exhaustive 한 **3 형태 (계층 아닌 순서 없는 메뉴)**: (base-trait-handle) `Arc<dyn KVCacheLayer>` / `Arc<dyn WeightLayer>`, (concrete-handle) `Arc<ConcreteLayer>` (예: `Arc<KIVILayer>` — register 시점 compile-time type 강제, downcast 0), (capability-handle) `Arc<dyn CapabilityTrait>` (Stage 측 정의, base trait 변경 0). **Cardinality 자유 (1/N/0 layer)** — 1 layer (signal-driven paradigm-specific) / N layer (cross-layer policy) / 0 layer (backend-only). 임의 1-layer-per-stage 가정 폐기. 위반 시 ctx god abstraction 재발생 + Stage 가 자기 책임 외 layer 접근 risk. | Correctness | static (코드리뷰), test | Stage struct 정의 source-grep 으로 `kv: &mut dyn ...` / `weights: &mut dyn ...` 등 ctx 통한 layer 접근 부재 확인. Stage 가 layer handle field (`Arc<dyn KVCacheLayer>` / `Arc<dyn WeightLayer>` / `Arc<ConcreteLayer>` / `Arc<dyn CapabilityTrait>`) 보유 패턴 PR checklist + handle 형태 분류 (kv/ vs weight/ vs system/) 의무화. |
