...
- **신규 INV-STAGE-ORDER-SAFETY** — 임의 submit-order crash-safe (순서=사용자 책임 / 안전=프레임워크 보장). safety over policy: ordering policy/config 미도입.
...
| **INV-STAGE-ORDER-SAFETY** (신규 2026-05-29) | arch/pipeline_stage_design_v2.md §5.3 (순서/안전), §8 | `PipelineStage` 는 임의의 submit-order 에서 crash-safe 해야 한다. 한 stage 가 다른 stage 의 선행 실행을 *가정* 하여, 그 가정이 깨질 때 panic/UB 가 나면 안 된다. 순서에 따라 *결과* 는 달라질 수 있으나(사용자 책임 — INV-DECODE-STAGE-005 와 짝), *crash* 는 안 난다(프레임워크 책임). 의미: 프레임워크는 안전만 보장하고, 여러 유효 순서 중 선택을 policy/config 로 금지하지 않는다 (safety over policy). 예: eviction→KIVI 든 KIVI→eviction 이든 둘 다 crash-free — 어느 쪽이 성능/정책상 나은지는 사용자가 정함. commutativity 강제 / named-phase ordering / priority 숫자 같은 ordering policy 는 미도입. 위반 시 특정 순서 조합에서만 재현되는 crash. | Correctness/Safety | static (코드리뷰), test | stage impl 이 선행 stage 실행을 가정하는 분기 (다른 stage 가 set 한 state 를 무조건 unwrap 등) source-grep + 방어적 처리 확인. PoC test 가 순서 permutation 일부에서 no-panic 검증. |
