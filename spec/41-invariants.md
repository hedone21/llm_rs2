...
| INV-LAYER-004 | 01-architecture SYS-102, SYS-104 | Cross-cutting 모듈(`observability/`, `resilience/`)이 L3 도메인의 concrete type을 import할 때는 trait/Sink 경유로 inversion한다. `EventSink`, `Transport`, `GpuEventMeter` 등이 표준 inversion 패턴. 예외(허용): `events::CacheEvent` enum이 pressure 결과를 직접 표현하는 경우 — enum이 inversion 매체이므로 동의어. 위반 예: `eval/eviction_hook.rs` → downcast `OpenCLBackend` (L1 concrete를 cross-cutting이 직접 소비). | Correctness | static, test | observability/resilience 모듈의 import 분석. concrete L1/L3 type을 import한 곳마다 trait inversion 가능성 검토. eval은 L4(`session/eval/`)로 격상 시 자연 해소. **Enum-as-data identifier 예외**: V-12(`events.rs::ActionResult`), V-10(`EvictMethod`) 등 enum 형태의 message data를 표현 매체로 보유하는 경우 §13.8-F에 따라 INV-LAYER-004 예외 허용. |
