...
| INV-DECODE-STAGE-006 | arch/pipeline_stage_design.md §3.3 (slim ctx), §8.1 (CTX-AUTHORITY) | `StageContext` **3 field** (`step` / `backend_ext` / `profiler`) mutation 권한 강제 X — code review 책임 (M1만). 특정 stage 가 자기 책임 범위 밖 field 를 mutate 하더라도 컴파일/런타임 검출 없음. PR checklist 의무화 — 신규 stage 추가 시 어느 ctx field 를 어느 phase 에서 mutate 하는지 명시 + reviewer 확인. **본 grill 2026-05-28**: 5 field (kv / weights 포함) → 3 field 로 축소 — kv / weights 책임 경계 위반 risk 자체가 ctx 에서 사라짐 (Stage 가 자기 보유 layer handle 만 접근). 권한 명확화. | Correctness | static (코드리뷰), test | PR checklist + stage 별 책임 매트릭스 (`arch/pipeline_stage_design.md` §5.3) 와의 정합성 검토. Phase β 진입 시 review template 도입. |
