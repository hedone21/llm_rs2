...
**현재 상태**: D2OHandler는 CachePressurePipeline 내 Handler로 구현 완료. **KvMergeD2o EngineCommand는 프로토콜 정의만 존재하며 (MSG-034b), action_registry에 미등록 상태이다.** 코드 구현이 필요하다.
...
### 왜 KvMergeD2o EngineCommand가 미등록인가

D2O의 merge 동작은 eviction과 동시에 실행되어야 하므로 CachePressurePipeline의 D2OHandler가 자연스럽다. 별도 EngineCommand로 분리하면 eviction과 merge를 개별 Directive로 전송해야 하여 원자성이 깨진다. 프로토콜에 정의만 존재하며, 향후 필요 시 action_registry에 등록할 수 있다.
