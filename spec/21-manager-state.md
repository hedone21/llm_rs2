...
Monitor 내부의 Level 판정 상태 머신이다. 원시 센서 값을 4-level (Normal/Warning/Critical/Emergency)로 변환한다.

> **Emergency Level의 역할**: Emergency는 Monitor Layer 전용 레벨이다. Manager의 OperatingMode(Normal/Warning/Critical)에는 Emergency가 존재하지 않는다. Emergency Level은 극단 조건(배터리 5% 이하, 온도 85°C 이상 등)을 PI Controller에 최대 압력(1.0)으로 전달하여 Supervisory가 Critical 모드로 진입하게 한다. D-Bus 전송 경로에서는 Engine이 Emergency Level을 직접 수신하여 자율적으로 Suspend할 수 있다 (SYS-055, SYS-085a 참조).
