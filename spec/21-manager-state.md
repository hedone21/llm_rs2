...
> **참고 (비규범)**: Manager와 Engine 간 모드 매핑:
...
> **참고**: `01-architecture.md` SYS-093은 4-state (Active 포함)를 도시하나, 코드에서 Active는 Connected의 논리적 하위 조건(Capability 수신 여부)이며 별도 상태가 아니다.
...
Monitor 내부의 보조 상태 머신이다. 원시 센서 값을 4-level (Normal/Warning/Critical/Emergency)로 변환하여 SystemSignal의 `level` 필드와 `reclaim_target_bytes` 등 보조 값을 산출한다.

> **Policy와의 관계**: Policy Layer는 `level`이 아닌 raw 필드(available_bytes, temperature_mc, cpu_usage_pct, battery_pct)에서 직접 압력을 계산한다 (MGR-021, MGR-024). ThresholdEvaluator의 출력은 D-Bus 전송 경로와 Monitor 내부 보조 계산에만 사용된다.
>
> **Emergency Level의 역할**: Emergency는 Monitor Layer 보조 레벨이다. Manager의 OperatingMode(Normal/Warning/Critical)에는 Emergency가 존재하지 않는다. D-Bus 전송 경로에서는 Engine이 Emergency Level을 직접 수신하여 자율적으로 Suspend할 수 있다 (SYS-055, SYS-085a 참조).
...
> **참고**: Energy hysteresis는 EnergyMonitorConfig에 필드가 없으며, 코드 내 고정값 2.0이다.
