...
> **참고 (non-normative)**: KvStreaming (C8)은 EngineCommand로 존재하나 ActionRegistry에 미등록 상태이다. StreamingLLM과 동일 동작(sink + sliding window)이며, 등록 시 C4/C5/C7과 상호 배타 그룹(eviction)을 형성해야 한다. ID C2, C3은 이전 설계에서 삭제된 액션에 할당되었으며 현재 미사용이다.
