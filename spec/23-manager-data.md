...
> (4) 핵심 데이터 타입: ActionId 10종, ActionKind, Domain, OperatingMode, PressureVector, ReliefVector, FeatureVector, ActionMeta, ParamRange, ActionParams, ActionCommand.
> (5) ActionRegistry 10종 액션 메타데이터 테이블, default_param_range, default_relief prior.
...
- ActionConfig, ActionRegistry 10종 액션 메타데이터
...
**[MGR-DAT-010]** SystemSignal은 Monitor Layer가 생성하여 mpsc 채널로 Policy Layer에 전달하는 도메인별 시스템 상태 메시지이다. **raw 센서 데이터에 더해 Monitor가 산출한 `level: Level` 필드를 항상 포함한다(MGR-DAT-015).** *(MUST)*

> **11번과의 관계**: `11-protocol-messages.md` MSG-100~104는 D-Bus 와이어 포맷을 정의한다. 내부 SystemSignal(이 문서)과 D-Bus SystemSignal(11번)은 동일하게 `level` 필드를 운반하며, D-Bus 경로는 SystemSignal의 Level을 그대로 직렬화한다(재계산 없음).
>
> **설계 원칙**: Monitor는 raw 센서 데이터 수집과 ThresholdEvaluator를 통한 Level 산출을 함께 수행한다. Policy Layer는 압력 계산에 raw 필드를 사용하며(MGR-021, MGR-024), Energy 도메인 등 일부 경로는 `level`을 직접 소비한다(MGR-DAT-014, MGR-ALG-015). 즉 `level`은 Monitor에서 산출되어 모든 소비 경로에 일관되게 전달된다.

**[MGR-DAT-011]** MemoryPressure — Memory 도메인 상태를 전달한다. *(MUST)*

| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 4종 | Monitor가 산출(MGR-DAT-015). 로깅/D-Bus 전파용. 압력은 raw 필드에서 직접 계산. |
...
| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 4종 | `m = level_to_measurement(level) * 0.5` → `max(pressure.compute, m)` → pi_compute (MGR-ALG-015). *(MUST)* |
| `reason` | EnergyReason | 6종 | 로깅 목적. |
| `power_budget_mw` | u32 | >= 0 | 전력 예산. 향후 활용 가능. |

> raw `battery_pct`는 `SystemSignal::EnergyConstraint`의 필드가 아니다. EnergyMonitor가 `/sys/class/power_supply/`에서 읽은 `battery_pct`를 `ThresholdEvaluator`로 `level`(4단계)로 변환하여 신호에 싣는다(MGR-019). Policy는 이 `level`을 소비한다. (2026-06-12 — MGR-ALG-015 divergence 해소와 함께 정합화.)

**[MGR-DAT-015]** Level enum — 4단계 심각도. **내부 SystemSignal의 모든 variant(MemoryPressure, ComputeGuidance, ThermalAlert, EnergyConstraint)에 `level: Level` 필드로 항상 포함된다.** Monitor가 raw 값에서 ThresholdEvaluator를 통해 Level을 산출하여 SystemSignal에 싣는다. `SystemSignal::level()` accessor가 임의 variant에서 Level을 추출한다(`shared/src/lib.rs`). D-Bus 와이어 메시지(`11-protocol-messages.md` MSG-100~104)도 동일 Level을 운반하며, SystemSignal의 Level을 그대로 직렬화한다(별도 재산출 없음). *(MUST)*
...
> **개정 이력 (2026-06-13)**: 구 명세는 "Level은 D-Bus 전송 경로 전용이며 내부 SystemSignal에는 포함되지 않는다"고 기술했으나, 실제 `shared/src/lib.rs::SystemSignal`의 4개 variant는 모두 `level: Level` 필드를 가지며 `SystemSignal::level()` accessor가 존재한다. 코드를 spec(Level 미포함)에 맞추는 것은 데이터 모델 파괴이므로, spec을 현 구현(Level 항상 포함)에 정합화하는 방향으로 정정했다. 동시에 MGR-DAT-010 본문/주석과 MGR-DAT-011/012/013 필드 테이블의 `level` 누락도 함께 정정해 §3.1 전체를 현 구현과 일치시켰다.
...
`action_name`은 ActionId의 snake_case 문자열이다: `switch_hw`, `throttle`, `kv_offload_disk`, `kv_evict_sliding`, `kv_evict_h2o`, `kv_evict_streaming`, `kv_merge_d2o`, `kv_quant_dynamic`, `layer_skip`, `swap_weights`.
...
**[MGR-DAT-040]** ActionId enum — 적응형 액션 식별자. 10종을 정의한다. *(MUST)*
...
| KvEvictStreaming | `kv_evict_streaming` | Memory | 구현 |
| KvMergeD2o | `kv_merge_d2o` | Memory | 구현 |
| KvQuantDynamic | `kv_quant_dynamic` | Memory | 구현 |
| LayerSkip | `layer_skip` | Compute | 구현 |
| SwapWeights | `swap_weights` | Memory | 구현 (mock + lua_policy) / hierarchical 통합 진행중 |

- serde: `rename_all = "snake_case"`. JSON 직렬화/역직렬화 시 snake_case 문자열 사용.
- `ActionId::from_str(s)`: snake_case 문자열 → ActionId 변환. 실패 시 None.
- `ActionId::all()`: 10종 반환 (SwapWeights 포함).
- `ActionId::primary_domain()`: SwitchHw / Throttle / LayerSkip → Compute, 나머지(KvOffloadDisk, KvEvictSliding, KvEvictH2o, KvEvictStreaming, KvMergeD2o, KvQuantDynamic, **SwapWeights**) → Memory.

> `types.rs`의 ActionId enum 정의(`#[cfg(feature = "hierarchical")]` 모듈)는 본 카탈로그와 1:1 일치해야 한다.
>
> **KvMergeD2o**: Engine 측에서는 `EvictMethod::D2o` 분기를 통해 CachePressurePipeline 내 D2OHandler를 재활용한다.
>
> **KvEvictStreaming**: StreamingLLM 변형. 별도 param `window_size`(인덱스 단위, MGR-DAT-056). 배타 그룹 `eviction`에 포함된다 (MGR-DAT-036, §6.1 예시 참조).
>
> **SwapWeights** (Dynamic Weight Swap, 2026-04-24 도입): primary dtype(F16/F32) → secondary dtype(현재 q4_0만 실행 가능)으로 layer 단위 weight를 런타임 교체하는 액션. ratio 파라미터(0.0~0.9)로 swap할 layer 비율을 지정한다. Engine 측에서는 `WeightSwapHandler`(arch/weight_swap.md §2.4 ENG-ALG-211)가 처리하며 IPC payload는 `EngineCommand::SwapWeights { ratio: f32, target_dtype: DtypeTag }`(MSG-042). 본 액션은 Memory 도메인 압력 완화를 1차 목적으로 하므로 primary_domain = Memory. lua_policy 직접 매핑 경로(`manager/src/lua_policy.rs::action_to_str`/`parse_single_action`)와 hierarchical 경로(ActionId::SwapWeights → ActionRegistry → EngineCommand::SwapWeights)는 동일 IPC 명령을 산출해야 한다(arch/weight_swap.md §2.8 참조).
...
**[MGR-DAT-054]** ActionRegistry 10종 액션 메타데이터 테이블 — ActionRegistry는 `from_config()`로 PolicyConfig에서 초기화된다. 설정에 포함된 액션만 등록된다 (최대 10종). 아래는 권장 기본 메타데이터이다. *(MUST)*
...
| KvEvictStreaming | Lossy | false | window_size | 64.0 | 2048.0 | eviction | Memory | 1.0 |
...
| SwapWeights | Lossy | false | ratio | 0.0 | 0.9 | (없음) | Memory | 1.0 |
...
>
> **SwapWeights reversibility 결정**: false. 일단 secondary dtype으로 swap된 layer를 primary로 되돌리려면 (i) primary mmap이 madvise(DONTNEED)된 페이지를 다시 fault-in해야 하고, (ii) 새 Arc 빌드 + ArcSwap::store가 필요하여 사실상 다시 한 번의 swap 비용이 발생한다. 또한 ENG-ALG-211 step (e)는 ratio_generation을 단조 증가시키므로 “Release를 RestoreDefaults 1회로 일괄 처리”하는 기존 Operation::Release 의미(MGR-DAT-053)와 직접 매핑되지 않는다. 따라서 Manager 액션 모델에서는 reversible=false로 분류하고, primary 복귀는 Engine의 `RestoreDefaults` 명령(MSG-040 계열)이나 별도 ratio=0 swap으로 처리한다. 향후 layer swap이 양방향으로 정착되면 v2에서 재검토.
>
> **SwapWeights param_range = [0.0, 0.9]**: 0.0은 NoOp(보수적, intensity=0), 0.9는 lua_policy.rs의 안전 clamp 상한과 일치한다(arch/weight_swap.md §2.8). ratio = 1.0은 lua_policy 경로에서 차단되며 hierarchical 경로에서도 동일 상한을 따라야 한다. Engine 측 ENG-ALG-211 자체는 ratio=1.0을 거부하지 않지만(전체 layer swap 가능), Manager 정책 표면에서는 안전 마진을 적용한다.
...
| KvEvictStreaming | window_size | 64.0 | 2048.0 |
...
| SwapWeights | ratio | 0.0 | 0.9 |

SwitchHw와 KvOffloadDisk는 파라미터 없는 액션이다 (param_range = None).

KvMergeD2o는 `default_param_range()`에서 keep_ratio [0.3, 0.9]을 반환한다.

KvEvictStreaming은 `window_size`(인덱스 단위, recent token 수)를 사용한다. keep_ratio 계열과 의미가 다르므로 같은 배타 그룹 `eviction`에 속하더라도 parametrize의 보수↔공격 방향은 동일하지 않다(intensity=1 → window_size=64로 공격적, intensity=0 → 2048로 보수적).

SwapWeights는 `ratio`(swap할 layer 비율)를 사용한다. min=0.0(NoOp, 완전 보수), max=0.9(lua_policy 경로의 안전 clamp 상한, arch/weight_swap.md §2.8). intensity=0 → ratio=0.0(보수적), intensity=1 → ratio=0.9(공격적). Engine 측 ratio_generation bump는 swap 결과가 비어 있으면 생략된다(arch/weight_swap.md §2.7), 따라서 ratio=0.0 명령은 안전하게 NoOp으로 동작한다.

> 파라미터 범위는 Engine의 물리적 제약에 의해 결정된다 (keep_ratio 0.3 미만이면 품질 붕괴, target_bits 4 미만은 양자화 정밀도 한계, swap ratio > 0.9는 본 코드베이스에서 정확성/안전성 sweep이 충분하지 않음 등). 향후 TOML 설정 가능하게 확장할 수 있다.
...
| KvEvictStreaming | 0.0 | 0.7 | 0.0 | 0.0 |
| KvMergeD2o | 0.0 | 0.6 | 0.0 | 0.0 |
| KvQuantDynamic | 0.0 | 0.3 | 0.0 | 0.0 |
| LayerSkip | 0.3 | 0.0 | 0.1 | -0.2 |
| SwapWeights | 0.0 | 0.5 | 0.0 | -0.2 |

이 테이블은 `21-manager-state.md` MGR-086과 동일하다. 데이터 관점에서 재기술한다.

> **SwapWeights prior 근거** (cold-start, 실측 calibration 전):
> - `memory = 0.5` — Llama 3.2 1B 기준 ratio=0.5 swap이 PSS 약 12% 감소를 목표(arch/weight_swap.md §5 Phase 실측 계획). 4D ReliefVector의 memory 차원이 [0,1]로 정규화되어 있으므로, “memory pressure를 약 0.5만큼 감소시킨다”는 도메인 prior로 0.5를 채택한다. KvEvict 계열(0.6~0.7)보다 보수적인 이유: KV는 압력 발생 시점에 대부분 가득 차 있어 evict 효과가 즉각적이지만, weight swap은 page fault-out 시점이 madvise 의존이라 변동성이 크다.
> - `latency = -0.2` — swap 직후 첫 token에서 plan rebuild + 일부 layer 재등록 비용으로 TBT 증가가 예상된다(arch/weight_swap.md §5 “TBT 증가 < 20%” 목표). KvOffloadDisk(-0.2)와 동일한 보수적 음수값을 채택. 학습기가 실측 데이터로 보정한다.
> - `compute = 0.0`, `thermal = 0.0` — weight swap의 1차 효과는 메모리이며, compute/thermal 부수 효과는 직접적이지 않다.
...
[policy.actions.kv_evict_streaming]
lossy = true
reversible = false
default_cost = 1.0
