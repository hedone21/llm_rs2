...
>
> **설계 원칙**: Monitor는 raw 센서 데이터를 수집·전달하는 것이 주 책임이다. `level` 필드는 Monitor 내부의 ThresholdEvaluator가 산출하는 보조 값으로, D-Bus 전송 경로와 `reclaim_target_bytes` 등 보조 계산에 사용된다. **Policy Layer는 `level`이 아닌 raw 필드에서 직접 압력을 계산한다** (MGR-021, MGR-024 참조).

**[MGR-DAT-011]** MemoryPressure — Memory 도메인 상태를 전달한다. *(MUST)*

| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 4종 | Monitor 보조 (reclaim_target_bytes 계산, D-Bus). **Policy 압력 계산에 미사용.** |
...
| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 4종 | Monitor 보조 (D-Bus). **Policy 압력 계산에 미사용.** |
...
| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 3종 (Emergency 없음) | Monitor 보조 (D-Bus). Emergency는 f64::MAX로 도달 불가. **Policy 압력 계산에 미사용.** |
...
> **참고**: `battery_pct` 필드는 현재 코드의 EnergyConstraint에 미포함이다. 코드 변경이 필요하다. 기존 코드는 `level`에서 압력을 계산하나, 설계 원칙(Monitor=raw, Policy=평가)에 따라 raw 값 기반으로 전환한다.
