...
**[MGR-DAT-010]** SystemSignal은 Monitor Layer가 생성하여 mpsc 채널로 Policy Layer에 전달하는 도메인별 시스템 상태 메시지이다. 4종 변형 각각은 `level: Level` 필드를 공통으로 포함한다. *(MUST)*

> **11번과의 관계**: `11-protocol-messages.md` MSG-100~104는 동일한 SystemSignal을 D-Bus 와이어 포맷 관점에서 정의한다. 이 문서는 Manager 내부에서 각 필드를 어떻게 소비·해석하는지를 정의한다. 필드 이름과 타입은 MSG-100~104와 동일하다.

**[MGR-DAT-011]** MemoryPressure — Memory 도메인 상태를 전달한다. *(MUST)*

| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 4종 | ThresholdEvaluator(Descending) 판정 결과. reclaim_target_bytes 계산에 사용. |
...
| `reclaim_target_bytes` | u64 | >= 0 | Level별 총 메모리 대비 비율: Normal=0, Warning=5%, Critical=10%, Emergency=20%. 현재 Policy Layer에서 미사용 (Engine에 전달되지 않음). |
...
| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 4종 | ThresholdEvaluator(Ascending) 판정 결과. |
| `temperature_mc` | i32 | 밀리섭씨 | `m = clamp(temperature_mc / 85000, 0, 1)` → pi_thermal.update(m, dt) (MGR-ALG-014). |
| `throttling_active` | bool | true/false | 현재 Policy Layer에서 미사용. 로깅/모니터링 목적. |
| `throttle_ratio` | f64 | [0.0, 1.0] | 현재 Policy Layer에서 미사용. 1.0=스로틀 없음, 0.0=완전 스로틀. |
...
| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 3종 (Emergency 없음) | ThresholdEvaluator(Ascending) 판정 결과. Emergency는 f64::MAX로 도달 불가. |
| `recommended_backend` | RecommendedBackend | Cpu/Gpu/Any | 현재 Policy Layer에서 미사용. SwitchHw 액션 결정에 향후 활용 가능. |
| `reason` | ComputeReason | 6종 | 현재 Policy Layer에서 미사용. 로깅 목적. |
...
**[MGR-DAT-014]** EnergyConstraint — Energy 상태를 전달한다. 별도 PI 인스턴스 없이 compute PI에 max-floor로 합산된다 (MGR-029). *(MUST)*

| 필드 | 타입 | 범위 | Manager 소비 |
|------|------|------|-------------|
| `level` | Level | 4종 | `energy_measurement = level_to_measurement(level) * 0.5` → `combined = max(pressure.compute, energy_measurement)` → pi_compute.update(combined, dt) (MGR-ALG-015). |
| `reason` | EnergyReason | 6종 | 현재 Policy Layer에서 미사용. 로깅 목적. |
| `power_budget_mw` | u32 | >= 0 | 현재 Policy Layer에서 미사용. Level별 기본값: Warning=3000, Critical=1500, Emergency=500. |

**[MGR-DAT-015]** Level enum — 4단계 심각도. Ord derive로 순서가 보장된다. *(MUST)*
...
| Warning | 1 | 경고 |
| Critical | 2 | 위험 |
| Emergency | 3 | 극한. Monitor Layer 전용. OperatingMode에는 없으며, Supervisory에서 Critical로 처리. |
