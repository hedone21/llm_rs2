...
**[MSG-030]** EngineCommand — Manager → Engine 개별 명령. `tag = "type"`, `rename_all = "snake_case"`. **13종 변형.** *(MUST)*
...
모든 액션 유도 상태를 기본값으로 복원한다. `active_actions`를 전체 초기화하고, throttle을 0으로 리셋한다.
