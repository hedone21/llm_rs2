# (이동됨) Manager 양방향 Unix Socket Transport 설계

> 이 파일의 내용은 `41_bidirectional_transport_design.md`로 이동됐습니다.
...
///
/// Drop 시 disconnect 신호를 보내고, thread는 EOF 또는 신호 수신 시 종료한다.
struct ReaderHandle {
    handle: JoinHandle<()>,
}
...
impl Drop for ReaderHandle {
    fn drop(&mut self) {
        ...
        // 강제 종료가 필요하면 stream을 shutdown하는 방식을 사용한다.
...
### 2-7. pipeline.rs 확장 (pipeline.update_engine_state)
...
impl PolicyPipeline {
    ...
    pub fn update_engine_state(&mut self, msg: &EngineMessage) {
        if let EngineMessage::Heartbeat(status) = msg {
            ...
            self.engine_state.values[1] = if status.active_device == "opencl" { 1.0 } else { 0.0 };
            self.engine_state.values[7] = if status.state == EngineState::Running { 1.0 } else { 0.0 };
        ...
        // Capability, Response는 별도 처리 (필요 시 확장)
    }
}
```
...
sequenceDiagram
    ...
    Note over M,E: 연결 수립
    E->>Ch: connect()
    Ch->>RT: spawn_reader(stream.clone())
...
| 리스크 | 심각도 | 완화 방안 |
|--------|--------|-----------|
| 기존 emit 동작 회귀 | 낮음 | UnixSocketEmitter 무수정. 기존 테스트 그대로 통과. |
| reader thread 누수 | 낮음 | ReaderHandle RAII로 drop 시 정리. Disconnected 전이 시 이전 handle 자동 drop. |
| inbox 백압력 누락 | 낮음 | `mpsc::sync_channel(64)` bounded channel로 자연 백압력 발생. |
| 재연결 시 중복 수락 | 중간 | `ensure_connected()`에서 상태 체크. 통합 테스트로 검증 필요. |
| engine_state 갱신 전 QCF 계산 | 낮음 | 단계적 적용: 1단계 수신 로깅 → 2단계 pipeline 연결. |
...
기존 `mock_engine` 바이너리를 그대로 사용하여 end-to-end 검증:
1. Manager 실행 (`--transport unix:/tmp/llm.sock`)
2. mock_engine 연결 → Capability + Heartbeat 전송
3. Manager 로그에서 `Engine heartbeat` 출력 확인
4. `pipeline.engine_state`가 zero가 아닌 값으로 갱신되는 것 확인 (debug 빌드)
