...
graph TB
    subgraph "Core (core/)"
        ...
        CacheManager["CacheManager<br/>pipeline + monitor + event_sink"]
        SystemMonitor["trait SystemMonitor<br/>+ mem_stats()"]
        EventSink["trait EventSink<br/>+ emit(CacheEvent)"]
    end

    subgraph "Pressure Pipeline (core/pressure/)"
        Pipeline["CachePressurePipeline"]
        HandlerTrait["trait CachePressureHandler"]
        EvictionHandler["EvictionHandler<br/>(wraps EvictionPolicy)"]
    ...
    subgraph "Integration"
        GenerateBin["generate.rs<br/>(eviction caller)"]
    end

    CacheManager --> Pipeline
    CacheManager --> SystemMonitor
    CacheManager --> EventSink
...
| 원칙 | 적용 방법 |
|------|-----------|
| **S** (Single Responsibility) | `KVCache`는 저장만, `EvictionPolicy`는 전략 판단만, `CacheManager`는 조율만, Model은 forward pass만 담당 |
| **O** (Open/Closed) | 새 전략은 `EvictionPolicy` 또는 `CachePressureHandler` trait 구현으로 추가. 기존 코드 수정 불필요 |
| **L** (Liskov Substitution) | 모든 policy/handler는 trait을 통해 완전히 교체 가능 |
| **I** (Interface Segregation) | `EvictionPolicy`: 3개 메서드, `CachePressureHandler`: 2개 메서드, `EventSink`: 1개 메서드 |
| **D** (Dependency Inversion) | `CacheManager`는 `CachePressurePipeline`에 의존, pipeline은 `dyn CachePressureHandler`에 의존 |
...
CacheManager는 내부적으로 항상 `CachePressurePipeline`을 통해 동작합니다.
`new()`로 생성 시 `EvictionPolicy`가 `EvictionHandler`로 래핑되어 pipeline에 등록됩니다.

```rust
// core/cache_manager.rs

pub struct CacheManager {
    pipeline: CachePressurePipeline,     // 항상 pipeline 모드
    monitor: Box<dyn SystemMonitor>,
    threshold_bytes: usize,              // 가용 메모리가 이 이하면 eviction 트리거
    event_sink: Arc<dyn EventSink>,      // 구조화된 이벤트 출력 (기본: NoOpSink)
}

impl CacheManager {
    /// Legacy API: EvictionPolicy를 EvictionHandler로 래핑하여 pipeline 등록
    pub fn new(policy: Box<dyn EvictionPolicy>, monitor: Box<dyn SystemMonitor>,
               threshold_bytes: usize, target_ratio: f32) -> Self;

    /// Pipeline API: 다중 CachePressureHandler 직접 구성
    pub fn with_pipeline(pipeline: CachePressurePipeline,
                         monitor: Box<dyn SystemMonitor>,
                         threshold_bytes: usize) -> Self;

    /// 각 generation step 후에 호출 (auto-eviction).
    ...
    /// Resilience 시그널 수신 시 호출. importance score 기반 eviction.
    pub fn force_evict_with_scores(
        &self, caches: &mut [KVCache], scores: &[f32],
    ) -> Result<EvictionResult>;

    /// score 기반 eviction 시도 (score 있으면 force_evict_with_scores, 없으면 force_evict).
    pub fn maybe_evict_with_scores(
        &self, caches: &mut [KVCache], scores: Option<&[f32]>,
    ) -> Result<EvictionResult>;

    /// EventSink 설정 (observability)
...
flowchart TD
    ...
    B["2. model.forward_into(...)<br/><i>score_accumulator captures attention weights</i><br/><i>(model은 forward pass만 수행, eviction 미포함)</i>"]
...
```
src/core/
├── kv_cache.rs         // KVCache (prune_prefix, memory_usage_bytes)
├── cache_manager.rs    // CacheManager (pipeline 기반 오케스트레이터)
...
│   ├── compress_handler.rs // CompressHandler (stub)
│   ├── quantize_handler.rs // QuantizeHandler (stub)
│   ├── merge_handler.rs    // MergeHandler (stub)
│   ├── swap_handler.rs     // SwapHandler (stub)
│   └── sparse_handler.rs  // SparseHandler (stub)
└── mod.rs              // 모듈 선언
```
