...
| `importance_i = max(0, 1 − cos)` | `engine/src/qcf/layer_importance.rs:219~220` |
| `O = Σα·V`, per-head | `engine/src/qcf/qcf_kv.rs:92, 134~149` |
| Head aggregation | `engine/src/qcf/mod.rs::aggregate_heads:108` |
| `ε_t = ‖W_pri − W_sec‖²_F / ‖W_pri‖²_F` | `engine/src/weight/noise_table.rs:307~318` |
| `ε_i = mean over 7 tensors` | `engine/src/weight/noise_table.rs:87~95, 109~134` |
| secondary dtype 조건 | `engine/src/weight/noise_table.rs:228` (`if info.dtype != DType::Q4_0 → skip`) |
| NaN ε layer 제외 | `engine/src/weight/noise_table.rs:131` (NaN 마킹) + `engine/src/weight/decider.rs:203~208` (분자/분모 filter) |
...
코드 위치:
- 통합 함수: `engine/src/qcf/qcf_kv.rs::compute_qcf_kv` (line 80)
- aggregation: `engine/src/qcf/mod.rs::aggregate_heads` (line 108)
...
코드: `engine/src/qcf/mod.rs:30~44`.
...
코드: `engine/src/qcf/layer_importance.rs:194~228`, `cosine_similarity()` line 269.
...
코드: `engine/src/weight/noise_table.rs::compute_tensor_epsilon` (line 218), 수식 line 307~318.
...
코드: `engine/src/weight/decider.rs::compute_qcf_weight_swap` (line 172), 내부 line 212~225.
...
코드: `engine/src/qcf/layer_importance.rs::ImportanceTable::compute_qcf_weight` (line 68).
...
| 항목 | 파일 | 함수/타입 | 위치 |
|---|---|---|---|
| QCF_kv 통합 함수 | `engine/src/qcf/qcf_kv.rs` | `compute_qcf_kv` | line 80 |
| KV 액션 enum | `engine/src/qcf/qcf_kv.rs` | `QcfActionType` | line 18 |
| Eviction `O^after` | `engine/src/qcf/qcf_kv.rs` | `compute_o_eviction` | line 360 |
| D2O merge `O^after` | `engine/src/qcf/qcf_kv.rs` | `compute_o_d2o_merge` | line 393 |
| KIVI quant round-trip | `engine/src/qcf/qcf_kv.rs` | `quantize_dequantize_f32` | line 524 |
| H2O retained 식별 | `engine/src/qcf/qcf_kv.rs` | `identify_retained_h2o` | line 479 |
| Head aggregation | `engine/src/qcf/mod.rs` | `aggregate_heads` | line 108 |
| QcfMetric 구조체 | `engine/src/qcf/mod.rs` | `QcfMetric` | line 30 |
| ImportanceTable | `engine/src/qcf/layer_importance.rs` | `ImportanceTable` | line 37 |
| Importance 측정 | `engine/src/qcf/layer_importance.rs` | `ImportanceCollector::record_after` | line 194 |
| Skip QCF | `engine/src/qcf/layer_importance.rs` | `ImportanceTable::compute_qcf_weight` | line 68 |
| OPR | `engine/src/qcf/layer_importance.rs` | `residual_norm_ratio` | line 246 |
| QuantNoiseTable | `engine/src/weight/noise_table.rs` | `QuantNoiseTable` | line 30 |
| ε_t 계산 | `engine/src/weight/noise_table.rs` | `compute_tensor_epsilon` | line 218 |
| ε_i 집계 | `engine/src/weight/noise_table.rs` | `new_from_frobenius` | line 73 |
| QCF_swap 본체 | `engine/src/weight/decider.rs` | `compute_qcf_weight_swap` | line 172 |
| Swap 결정 | `engine/src/weight/decider.rs` | `WeightSwapDecider::decide` | line 57 |
| ΔPPL estimator | `engine/src/qcf/estimator.rs` | `DegradationEstimator::estimate` | line 136 |
