...
- **GATE-C (다음 단계, device-gated)**: cdylib 승격 + dlopen/`register_plugin` + `extern "C"` vtable. PluginManifest/libloading 인프라 0% 착수 → 별도 단계.
  - **→ ADR-0009 가 GATE-C 설계를 확정**(panic abort-parity / opaque-handle vtable / 가산 registry / plugin-arena plan 마샬링 / `--load-plugin` / plan-identity 게이트). **정정**: GATE-C 를 일괄 "device-gated"로 본 것은 부정확 — **Stage 축 GATE-C 는 host-implementable**(plan-only, plan-identity 게이트는 GPU 불필요). device-gated 는 **Format 축 encoder perf**(v2)에 한정.
