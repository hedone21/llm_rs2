...
| **RestoreDefaults swap 역전 = 신규 메커니즘** — `SwapExecutor` 단방향(F16→Q4_0), F16 recall 코드 전무. skip→0/partition 역전과 난이도 질적 차이 | 中 | ~~별도 결정 항목으로 분리~~ → **착수 (2026-06-13, 사용자 결정 B)**. §6 참조. 재검증 결과 위험 하향: `SwapExecutor.target_dtype`이 dtype-generic이라 "F16 recall 코드 전무"는 부정확(executor는 양방향 지원, 가드만 Q4_0-only). 신규 `RecallWeights` OneShot이 "이미 Consumed된 OneShot 역기동"을 비문제화(새 directive=새 Stage). F16 payload=AUF multi-dtype 상주(dual-resident 신규 구현 불요). 잔여 위험 = F16 view 바인딩 reject 분기 loud no-op(INV-195) + device 정확성 게이트(INV-194) |
...
- ~~**swap reversal**(F16 recall)~~ → **착수 (2026-06-13, 사용자 결정 B = 옵션 B "명시 트리거 F16 recall")**. spec 의미 변경(단방향 가정 완화)이 승인됨. **메커니즘 설계 요지**:
  - **directive 표면**: 신규 `EngineCommand::RecallWeights { ratio }`(MSG-043). RestoreDefaults 확장이 **아니다** — RestoreDefaults는 wire 안정 + De-escalation SEQ(SEQ-060~064)·verify 시나리오가 의존하는 "묶음 reset"이라 의미 오염 금지. recall은 secondary I/O + plan invalidation을 동반하는 의도적 무거운 작업이므로 명시 directive가 정직. **평시 RestoreDefaults swap no-op은 현행 유지**(production winner Incremental의 Q4 유지 이득 보존 — 사용자 결정 보존, INV-192).
  - **역방향 경로**: `SwapExecutor` **재사용**(target_dtype=F16) + 별도 OneShot `WeightRecallStage`(`WeightSwapStage`의 역방향 형제). `SwapExecutor.target_dtype`은 도입 시부터 dtype-generic 필드라 인터페이스 변경 0 — 막던 가드는 `dtype_tag_to_dtype` Q4_0-only(INV-126)와 `ReverseSwapRejected`(production swap 안전 가드)뿐이며 **방향별로 분리**하여 우회. "이미 Consumed된 OneShot의 역기동"은 신규 RecallWeights directive가 새 OneShot Stage를 submit하므로 비문제(전방향 swap과 동일한 directive→OneShot 모델).
  - **swap된 layer 추적**: dispatcher sticky 상태가 아니라 **엔진 측 `model.layers[i].current_dtype()`** SSOT 재사용(swap precision은 transient라 dispatcher 측 추적 부재 — "현재 Q4_0인 layer"가 곧 recall 후보).
  - **F16 payload 소스**: AUF v0.2 multi-dtype secondary(ENG-DAT-097) 또는 W-AUF-2 self-secondary에 F16 variant가 상주 → `SecondaryDtypeChoice::F16`로 F16 view 별도 바인딩(같은 mmap 공유, INV-125). 신규 데이터 소스 0.
  - **불가 분기 UX**: loud no-op(stderr 1회 + graceful Consumed, INV-195) — F16 variant 부재(`DtypeNotFound`) / Adreno SOA 경로(`AdrenoSoaF16Rejected`, SOA=Q4_0 전용이라 recall 원천 차단) / no_secondary / swapped 0개 / in-flight. silent 금지.
  - **manager 자동 트리거**(품질 메트릭) 미정의 — 이번 범위는 메커니즘 + 수동/시나리오 발화(mock_manager `RecallWeights`) 검증까지. policy 자동 발화는 품질 메트릭 도입 후속.
  - 대응 spec: ENG-ALG-240/241(§3.12.23), MSG-043, SEQ-065~067, INV-192~195, INV-126 완화. arch: `pipeline_stage_design_v2.md §5.6.8`(신설).
- **RestoreDefaults 합성 역전**(partition + precision 동시) — **Deferred 유지 (recall 범위 제외)**. recall은 precision 축(F16↔Q4_0)만 다룬다. partition(format×hardware 곱)의 처분은 `RestoreDefaults`가 별도 Full 복원(`submit_partition_full`, AB-4)하며 recall과 직교 — 동시 적용 시 각자 자기 축만 역전한다. 두 축 합성 역전(단일 directive로 partition+precision 동시 복원)은 별도 결정으로 분리.
