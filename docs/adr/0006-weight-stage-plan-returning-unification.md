...
- **OneShot multi-tick**: 증분 drain 동안 매 `PreSwap` tick `drain_chunk` 후 `StageOutcome::Continue`(GC 아님, pipeline.rs:115), `is_done()` 되는 tick 에 `Consumed` 반환 → GC. 현 live retire(`Option::take()`) 자리 = `Consumed` 자리. "1회 실행 후 GC"(pipeline.rs:98)는 "1 plan = 1 OneShot 생명, 여러 tick Continue 후 종료 시 Consumed"로 ADR 이 의미 고정.
...
- **4 swap mode**(Incremental/IntraForward/PhaseAware/LayerImmediate, swap_runtime.rs) 의 단일 `WeightStage` 흡수 vs mode 별 executor 다형성.
