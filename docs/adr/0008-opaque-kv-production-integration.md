...
> - ✅ **부수 발견 → 수정(`00918e9e`): `--kv-format q4_0`(typed) + h2o force_evict mid-decode SIGSEGV**. root cause = `KVCache::shrink_to_fit()` 의 `copy_slice` 가 count/offset 을 element 단위로 넘겨 Q4_0(type_size=`sizeof(BlockQ4_0)`=18B) 에서 ~QK4_0(32)배 OOB read. `grow()` 는 element→block 변환했으나 `shrink_to_fit()` 누락. 경로 = eviction → `release_unused_pages`(underutilized) → `shrink_to_fit`. f16/f32(element=정답)·opaque(byte 회계) 무사 → q4_0 typed 한정(opaque fix 와 무관, pre-existing). fix = SeqMajor·HeadMajor block 변환 + 회귀 테스트 2종. lib 1260/0, e2e EXIT 0(Shrunk 512→256, final_pos 215).
