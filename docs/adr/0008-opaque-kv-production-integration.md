...
> **Status**: Accepted · **구현 완주 2026-06-08** (Stage 1 `6b4eaef5` / Stage 2 `d1b6cb55` / Stage 3 `114b9b12`; lib 1258/0 + clippy clean; `--kv-format synth_q4` == `q4_0` token-identical Qwen2.5-1.5B CPU)
> **Date**: 2026-06-08
...
- **e2e eviction 런 (bin 배선)** — opaque eviction primitive(prune/shift/shrink/merge)는 unit 게이트로 bit-identical 증명됐고 production eviction 파이프라인은 dtype-agnostic(primitive 만 호출)이라 구성상 동작하나, **CLI e2e 런은 미검증**: `argus_cli`(--kv-format 보유)는 happy-path 전용(`eviction_policy=none` 강제), `legacy_generate`(eviction 보유)는 --kv-format 미배선. 필요 시 둘 중 하나 배선(별 작업, eviction 은 장컨텍스트라 device-gated).
