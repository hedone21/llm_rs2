...
- **stage = "planning" 표면** — 기법은 순수 계산(score + K 스냅샷 → keep-list + 가중 merge 계획)을 반환하고 **엔진이 실행**(compaction + 가중 합산). 버퍼를 직접 안 만진다. h2o·sliding·streaming·h2o+·**d2o**(merge 포함)를 한 표면으로 덮는다.
...
- **linkme > inventory > 명시 카탈로그**: inventory는 life-before-main 생성자라 `panic="abort"`/Android에 위험. 명시 중앙 카탈로그(model 1)는 "폴더만 떨구기"를 깬다. linkme는 순수 링커 데이터(생성자 0)라 안전하고, crate Cargo 의존이 링크를 보장해 linkme의 dead-crate 함정을 제거한다. 남은 fat-LTO section-strip 리스크는 self-test로 관리.
...
- 확장 trait 표면: `engine/src/pressure/eviction.rs:13`(`EvictionPolicy`), `engine/src/format/kv_cache_format.rs:57`(`KVCacheFormat`+`Merge`:46), `engine/src/backend.rs:128`(`Backend`)
