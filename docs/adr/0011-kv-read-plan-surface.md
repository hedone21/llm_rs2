...
> **Status**: Proposed → **Accepted (S2 dispatch 전략 = Amendment A1, 2026-06-12 — 구현 착수)**. 분기 취소(2026-06-12)로 S1~S6 구현 진입. 표면(D1~D6)은 본문 고정, hot-path dispatch 는 Amendment A1.
> **Date**: 2026-06-12
...
## Amendment A1 (2026-06-12) — S2 hot-path dispatch 전략

> **Status**: Accepted (구현 착수 게이트). **Trigger**: §7 R1(RPN 378)이 "hot-path dispatch 전략(정적 vs dyn)은 구현 ADR-amendment 로 별도 고정"이라 위임 → 분기 취소(2026-06-12)로 S1~S6 구현 진입. 본 amendment 가 S2(엔진 executor seam)의 dispatch 메커니즘을 고정해 R1 을 닫는다.
> **Decision-makers**: Architect (Backlog Burndown T4, `.agent/todos/sprint_backlog_burndown_2026_06_12.md`).
> **실코드 정독 근거**: `engine/src/models/transformer.rs::forward_into`(decode layer loop L1489~1640, INV-147 hook 선례 L1496/L1628), `forward_into_offload`(decode arm L2700~), `engine/src/layers/transformer_layer/forward_gen_fmt.rs`(`fmt.attention_into` 호출 L166), `engine/src/format/kv_cache_format.rs`(`attention_into` 시그니처 L94), `engine/src/kv/eviction/stage_registry.rs`(`KVStageCtx`/`execute_kv_plan`/`AbiStageCtx` 선례).

### A1.1 결정 요약 (4 결정 포인트)

| 결정 포인트 | 결정 | 근거 |
|---|---|---|
| **(a) read stage 부재 시 비용** | **per-step 1회 캐시** — read stage 핸들을 forward 진입 시 `Option<&dyn KVReadStage>` 로 1회 받아 layer loop 전체에서 재사용. `None` 이면 layer loop 가 분기 0(기존 경로 byte-identical). | per-layer `find_read_stage` 호출 금지(N×/token = R1 직격). INV-147 의 `layer_boundary_hook: Option<&dyn>` 가 args 슬롯으로 1회 주입되는 패턴과 동형. |
| **(b) 활성 시 dispatch** | **`&dyn KVReadStage` (Box 아님) 정적 분기 + dyn call/layer** — `read_plan(ctx)` 는 layer 당 1회 호출. KVCacheStage/WeightStage 선례(Box<dyn> step-tier)와 달리 read 는 layer-tier 라 호출 빈도가 N× 높지만, **page-granularity 가 layer 당 1 dyn call 로 묶어** op-tier(per-token-per-head) 가 아니다. happy path(부재)는 dyn call 0. | ADR §5 (E) 기각이 "layer-tier 호출은 받되 INV-HOTPATH-DISPATCH 정합"을 요구. layer 당 1 dyn call 은 attention(그 layer 의 수십 op) 대비 무시 가능. 정적 monomorphize 는 `forward_into` 가 이미 `&[Arc<dyn KVCacheFormat>]` trait-object 경로라 불가능·불필요. |
| **(c) plan 수명/소유** | **per-step 재사용 버퍼(`KVReadPlan` 1개를 forward 인자로 빌려 layer 마다 in-place 갱신)** — layer 마다 `Vec<usize>` 신규 할당 금지. read stage 가 `&mut plan.select` 를 받아 clear+extend(capacity 재사용)하거나, 첫 구현은 plan 을 stage 내부 `&self` Mutex 버퍼로 보유하고 ctx 가 `&[usize]` borrow 만 노출. | ADR §6 "read plan 은 캐시를 변형하지 않는다" → plan 은 read-only 소비라 수명이 그 layer attention 으로 한정. ADR-0004 executor 가 plan 을 즉시 소비 후 drop 하는 선례. Tier 1(정확성)은 단순 `Vec` 신규 할당 허용(아래 A1.4 단계적 적용). |
| **(d) ReadStageCtx** | **기존 `StageCtx` 재사용 (신규 ctx trait 0)** — ADR §11 명시. read stage 는 `tensor(Key)`/`tensor(QueryStats)`/`current_pos`/`n_kv_heads`/`head_dim` 만 읽으며 이 전부가 `StageCtx` 에 이미 존재. `KVStageCtx`(stage_registry.rs:229)를 그대로 read stage 에 넘긴다. C-ABI 도 `AbiStageCtx` 재사용(ADR §11 MQ-5). | ADR §11: "read stage 전용 ctx 신설 불요(기존 StageCtx 가 충분)". `target_len`(budget)만 read 에 의미 없으나 무시(읽지 않음). 표면 수 절약 + AbiStageCtx 마샬링 무수정. |

### A1.2 absolute constraint (C5 보수적 디폴트)

happy path = **read stage 부재**(`--read-stage` 미지정, 빌트인 0개 시작). 이 경로는:
- **분기 0 또는 1회** — `forward_into` 가 `read_stage: Option<&dyn KVReadStage>` args 슬롯을 받고, layer loop 안에서 `if let Some(rs) = read_stage` 1회 분기. `None` 이면 그 분기 false → 기존 `fmt.attention_into(full read)` 로 직행. INV-147 의 `layer_boundary_hook` 와 byte-identical 한 분기 패턴.
- **기존 경로 100% 보존** — read stage 부재 시 `forward_gen_fmt`/`forward_prefill_fmt` 의 `attention_into` 호출 인자·순서·값이 1바이트도 안 변한다. α-K frozen byte-identical 게이트 대상.
- **dispatch 메커니즘은 args 슬롯 추가 only** — `KVCacheFormat` base trait(6-method) 무변경. `SelectiveRead` 는 별도 capability trait(D4). base trait 표면에 select 누출 0.

### A1.3 seam 위치 (정확한 파일:함수)

S2 의 read_plan 호출 + 라우팅 seam 은 **2지점** — `forward_into` decode arm 과 `forward_into_offload` decode arm. prefill arm 은 Tier 1 범위 밖(decode-only, ADR §10 "prefill TTFT" 는 후속).

```
engine/src/models/transformer.rs
  forward_into()  decode arm (현 L1565 `} else if is_decode {`)
    ① layer loop 진입 전: read_stage = args.read_stage  (per-step 1회 캡처, A1.1(a))
    ② layer i attention 직전(forward_gen_fmt 호출 전):
         if let Some(rs) = read_stage {                       // 분기 1회(INV-147 동형)
             let ctx = KVStageCtx::new(&cache_i, 0, None, None, None, qs_i);  // (d) 재사용
             if let Some(plan) = rs.read_plan(&ctx) { ...라우팅... }          // (b) dyn call/layer
         }
    ③ 라우팅: plan 을 forward_gen_fmt 에 select 인자로 전달 → fmt 가 SelectiveRead 면
         attention_into_selected, 아니면 plan 무시 + attention_into(full read 폴백, D4)

  forward_into_offload()  decode arm (현 L2700 `if is_decode && workspace.is_some() {`)
    동일 패턴 + plan.select 를 prefetch 큐 공급원으로도 전달(S6, 배선 only)
```

**중요 — 핸들 borrow 충돌 회피**: `read_plan(ctx)` 의 ctx 는 `&KVCache`(또는 `&dyn KVCacheFormat` 위 read accessor)를 immutable borrow 한다. 그러나 `forward_gen_fmt` 는 `fmt: &Arc<dyn KVCacheFormat>`(`&self` mutation, `attention_into` 가 `&self`)라 read_plan 의 immutable borrow 와 충돌하지 않는다(둘 다 `&self`). plan 산출(borrow 종료) → forward_gen_fmt 호출(plan 을 값/slice 로 전달) 순서로 KVStageCtx 의 immutable borrow 가 forward 전에 끝난다(stage_registry.rs:328 `StageBackedPolicy::run` 의 "plan 산출 후 borrow 종료, 이후 executor `&mut`" 패턴과 동형).

### A1.4 단계적 적용 (R1 완화 = frozen 게이트 + 점진 최적화)

1. **Tier 1 (정확성, S2~S5)**: read stage 부재 happy path 는 분기 1회로 고정(절대 제약). 활성 경로는 plan `Vec<usize>` 신규 할당 허용(layer 당 1회 alloc — 정확성 우선, 성능 주장 안 함). 게이트 = 부재 시 byte-identical + 활성 시 폴백 bit-identical.
2. **Tier 2 (할당 최적화, 후속)**: plan select 버퍼 재사용(A1.1(c) 재사용 버퍼)로 layer 당 alloc 제거. TBT Δ≤+3% 측정으로 검증. 이 최적화는 8B/offload 측정과 함께 진행(ADR §10 — 1B 성능 주장 금지).

### A1.5 R1 closure (어떻게 amendment 가 R1 을 닫는가)

R1(layer-tier hot-path 회귀, RPN 378)의 완화책 4개가 amendment 로 구체화됨:
- "부재 시 분기 1회" → A1.1(a) per-step 캐시 + A1.2 args 슬롯 = **INV-147 byte-identical** 게이트로 강제(측정 가능).
- "page-granularity 가 호출 빈도 낮춤" → A1.1(b) layer 당 1 dyn call(op-tier 아님) 명문화.
- "정적 vs dyn 구현 ADR-amendment 위임" → A1.1(b) `&dyn` layer-tier 1 call + happy path dyn 0 으로 **확정**(위임 해소).
- "α-K frozen byte-identical + TBT Δ≤+3%" → A1.4 Tier 1/2 게이트로 분리. 잔여 RPN: 활성 경로 alloc 비용은 Tier 2 로 defer(1B 무이득 = ADR §10 예측됨, 실패 아님).

**잔여 리스크**: A1.1(b) 의 "layer 당 1 dyn call" 이 fat-LTO `.so` 단계(ADR-0005 §8 L2)에서 인라인 불가가 되면 회귀 가능. 그러나 (1) 빌트인 Quest 는 정적 링크(dyn call 인라인 후보), (2) `.so` read stage 는 미존재(첫 빌트인만), (3) TBT Δ≤+3% 게이트가 잡는다. R1 RPN 은 amendment 후 **S(9)×O(3 — 분기 1회 강제)×D(4 — frozen 게이트 탐지)= 108** 로 강하(완화 전 378).
