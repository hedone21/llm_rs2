...
3. **마이그레이션 순서** (R-G4 plan 후속):
   ...
   - **[갱신 주 2026-06-04, (3p)/(4) ④-a viability cut — wf `wf_00283de8-f1a` 소스 직접 재확인]** 위 (3p)/(4) 표기 2건을 정정한다(SSOT = `arch/pipeline_stage_design_v2.md` §9.1 의 동일 정정 cross-ref).
     ...
     - **[갱신 주 2026-06-04, BC 완주 step ① 분해]** 사용자 BC 완주 결정의 step ①(B-2/B-4 cold-path flip)은 SSOT §9.1-BC1 에서 *각각 runnable+gated* 인 2 증분으로 분해됐다: **①-a(B-4 eval) = 첫 증분**(`run_eval_ll_generic<C>` + `StepHook<C>`/`CacheSnapshot<C>` 의 `C` 다형성 → enum/dyn, KVCache·KiviCache 2-variant; forward entry·hot path·legacy·DecodeLoop 미접촉; host EvalOutput JSON bit-identical 게이트, device 불요) → **①-b(B-2 prefill)**(prefill batch entry flip — decode-only `forward_into_fmt` 게이트로 못 닿음 + prefill 부가기능 다수 + `write_kv_batch` GPU batch scatter 미흡수(C3) 라 (3a)/(3b)/C3 선결 후 신 `forward_into_fmt_batch` 로 처리, device 게이트). 첫 증분이 eval 인 이유 = forward entry 미접촉·cold·host 결정적 검증. Implementer 계약(수정 파일/게이트 명령/성공 기준) = SSOT §9.1-BC1-CONTRACT.
