...
> **Selected**: 갈래 2 — Trait object (`Arc<dyn KVCacheLayer>`)
...
- **Q27 (Mixed storage 허용)**: Layer 별 다른 KV storage paradigm 허용 (layer 0 = KIVI Q4, layer 1 = TurboQuant, ...)
- **Q31 ((γ) Layer handle 전환)**: PipelineStage 가 `Arc<dyn KVCacheLayer>` 를 register 시점 보관 — Stage 별 책임 layer 만 보유 (god ctx 회피)
- 5년 시야: 새 KV paradigm (SnapKV / D2O / TurboQuant / KIVI variants / Sparse / KvOffload 등) 추가 frequency 가 증가 추세 — 매 paradigm 추가 마다 generic instantiation explosion 발생

기존 Generic monomorphization 정책은 (a) mixed storage 차단, (b) `Arc<dyn KVCacheLayer>` 의 Stage 객체 보관 패턴 차단 (Generic generic parameter 가 trait object 로 들어갈 수 없음), (c) 매 paradigm 추가마다 forward path 전체의 instantiation 증가. 본 ADR 은 이 trade-off 를 재평가하여 정식 paradigm 전환을 결정한다.

---

## 2. Decision

**KV cache dispatch 를 Generic monomorphization (`<C: KVCacheOps>`) 에서 Trait object (`Arc<dyn KVCacheLayer>`) 로 전환한다**.

상세:
- `KVCacheOps` trait (현 ~15 method) → `KVCacheLayer` trait (5 method: idx / current_pos / capacity / view / 3 mutation + apply_storage). 자세한 시그니처는 `arch/pipeline_stage_design.md` §3.5.
- Forward path 의 `<C: KVCacheOps>` generic parameter → `&[Arc<dyn KVCacheLayer>]` slice.
...
- KVCacheOps 의 15 method 중 read-only 일부 → KVCacheView sub-trait (KVCacheLayer::view 반환).
...
본 grill 결정 #4: layer 별 다른 KV storage paradigm 허용 (KIVI per-layer mix / D2O variance allocation / SnapKV / 향후 Sparse 등). 이는 paper-aligned use case (D2O Eq.10/11 layer-level variance allocation, KIVI partial layer Q2, ...) 를 직접 지원하기 위한 핵심 요구사항.

Generic monomorphization 은 model build 시점에 한 concrete type 으로 고정되므로 layer 별 다른 paradigm 불가. Trait object 만이 이를 가능케 함.

### 3.2 Q31 (γ) Layer handle 모델

본 grill 결정 #8: PipelineStage 가 register 시점에 `Arc<dyn KVCacheLayer>` 를 보관. Stage 가 자기 책임 layer 만 보유 (god ctx 회피).
...
### 3.3 5년 시야 paradigm frequency

본 프로젝트 git history (2026-03 ~ 2026-05) 에서 KV paradigm 추가 frequency:
...
매 paradigm 추가마다 generic instantiation 증가 — `LlamaModel<KVCache>` / `LlamaModel<KiviCache>` / `LlamaModel<...>` ... binary size 증가 + 컴파일 시간 증가. Trait object 는 paradigm 추가가 binary size 0 (impl 만 추가).

### 3.4 LayerSlot/RCU 자연 정합

본 프로젝트 `LayerSlot::rcu_weights` (slot.rs:158) 는 이미 `&self` mutation via RCU semantics. KVCacheLayer trait 도 동일 패턴 (interior mutability via `&self`) — 새 정신 도입 cost 0.

### 3.5 Mixed storage 요구사항의 PipelineStage 정합

본 grill PipelineStage hook 패턴 (2026-05-27 결정) 에서 EvictionStage / SwapDispatchStage / KvMergeStage 등이 layer 단위로 책임 분배 — Stage 가 자기 책임 layer 의 storage paradigm 을 추상화 통해 추론 — Stage 가 storage-agnostic (INV-KVCACHELAYER-PRIMITIVE-AGNOSTIC). 이는 trait object 의 storage-agnostic dispatch 와 직접 매칭.
...
- **Mixed storage 가능**: layer 별 다른 KV storage paradigm (Q27 #4 직접 지원)
- **새 paradigm 추가 cost 감소**: impl 만 추가, 기존 forward path 변경 0. 매 paradigm 마다 ~20 file refactor 회피.
- **PipelineStage 패턴 자연 정합**: Stage 객체 안 `Arc<dyn KVCacheLayer>` 보관 가능 — (γ) 모델의 핵심 인프라.
- **Stage storage-agnostic**: stage 가 dtype / codebook / rotation matrix / sparse pattern 모름. OCP 강화.
- **Binary size 감소**: generic instantiation 폭증 회피 (5 paradigm × 모델 = 5 binary blob 회피)
...
- **Bit-identical 검증 (R-G5 RPN 168)**: KVCacheOps 15 method → KVCacheLayer 5 method 통합 (compact merges/keep atomic) 으로 D2O / SnapKV / KIVI 의 출력 비트 정확성 회귀 risk. Phase α-K 종료 게이트 (S25 Qwen2.5-1.5B Q4_0 32 token bit-identical) 필수.
...
- **Contiguous slice access 손실**: `&mut [C]` (generic) → `&[Arc<dyn KVCacheLayer>]` (trait object). interior mutability 로 `&self` mutation 이라 `&mut` 불필요하나, slice 의 cache locality 일부 손실 가능 — 측정 통해 검증.

### 4.3 Phase α-K 진입 조건

- Phase α-W 종료 + 본 ADR Accepted 상태
- KVCacheOps 15 method → KVCacheLayer 5 method 매핑 표 작성 (R-G2 mitigation)
...
**형태**: 현 `KVCacheOps` generic 유지. Layer 별 다른 paradigm 차단.
...
- 5년 시야 paradigm frequency 증가 시 generic instantiation explosion.

### 5.2 갈래 2 — Trait object (`Arc<dyn KVCacheLayer>`) (**ACCEPTED**)

**형태**: 본 ADR 의 결정. KVCacheOps generic → KVCacheLayer trait object.
...
### 5.4 갈래 4 — Static enum dispatch (`enum KVCacheLayerVariant { Standard, KIVI, Sparse, ... }`) (REJECTED)

**형태**: Trait object 대신 enum variant 로 static dispatch. 새 paradigm = 새 enum variant.

**거부 사유**:
- 새 paradigm 추가 = enum variant 추가 = **OCP 위반**. 본 grill 의 hook 패턴 (PipelineStage) 이 OCP 정신 채택했으므로 KV dispatch 도 동일 정신 — Trait object 가 OCP 정합.
- Variant 추가 시 모든 caller 의 `match` exhaustive 업데이트 필요 — 본 grill 의 stage 객체 캡슐화 정신과 충돌.
- Variant 수가 paradigm 수로 한계 — extension 없는 closed type system.
...
- **S25 Qwen2.5-1.5B Q4_0 32-token bit-identical**: baseline (HEAD `master`, Generic dispatch) vs Phase α-K branch (Trait object) token id sequence 100% 일치 (모든 KV paradigm — Sliding / H2O / D2O / KIVI / SnapKV).
- 측정: `python scripts/run_device.py -d s25 generate --backend opencl --opencl-rpcmem --model-path qwen2.5-1.5b-q40.auf --prompt "..." --max-tokens 32 --seed 42` × 5 KV paradigm.
...
- 임계 초과 시 R-G1 vtable cost 직접 검증 — flamegraph profile 로 KVCacheLayer trait dispatch overhead 측정.
...
- **R-G2 매핑 표 완성**: KVCacheOps 15 method 전수 → KVCacheLayer 5 method + KVCacheView sub-trait + 폐기 분류.
...
- KVCacheOps → KVCacheLayer 매핑 표에서 semantic loss 5+ method (R-G2)

---

## 7. References

- `arch/pipeline_stage_design.md` §3.5 (KVCacheLayer 시그니처), §10 (Phase α-W → ADR-0001 → Phase α-K), §14.1 Q32 (갈래 비교)
...
2. **KVCacheOps trait 보존 기간**: Phase α-K 중에는 KVCacheOps + KVCacheLayer 둘 다 존재 (점진적 마이그레이션). KVCacheOps trait 의 폐기는 Phase α-K 종료 + bit-identical 검증 후 별 commit.
3. **마이그레이션 순서** (R-G4 plan 후속):
   - (1) KVCacheLayer trait 정의 + KVCacheView sub-trait + StandardLayer / KIVILayer impl
