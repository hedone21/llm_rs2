...
3. **마이그레이션 순서** (R-G4 plan 후속):
   ...
   - (3p) **plan path** generic → trait object (`backend/opencl/plan.rs::execute<C: KVCacheOps>` → `attention_into` 배선) — **production hot layer-tier crux**. (4) 의 선결 조건(plan-dep).
   - (4) bit-identical 검증 → KVCacheOps trait 폐기 commit — **(3p) 선결 필수**(plan path 가 `KVCacheOps` 의 마지막 generic 소비자, 미flip 시 폐기 컴파일 차단).
