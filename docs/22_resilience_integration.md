...
└──────────────┬───────────────┘
               ...
               ▼
┌──────────────────────────────┐
│   DbusListener Thread        │  std::thread::spawn
│   - zbus blocking loop       │
│   - parse → SystemSignal     │
│   - tx.send(signal)          │
└──────────────┬───────────────┘
...
│   - rm.poll() per token      │  try_recv() → 0 blocking
│   - execute actions          │
│   - throttle via sleep       │
└──────────────────────────────┘
```
...
use llm_rs2::resilience::{
    DbusListener, ResilienceManager, InferenceContext,
...
let mut resilience_manager = if args.enable_resilience {
    ...
    let _listener_handle = listener.spawn();  // D-Bus 스레드 시작
    println!("[Resilience] Manager enabled — listening for D-Bus signals");
        ...
        if let ResilienceAction::Evict { target_ratio } = &action {
            ...
            if remove > 0 {
                ...
                eprintln!(
                    ...
                    remove, kv_caches[0].current_pos + remove, kv_caches[0].current_pos
                );
            }
        } else {
...
| Action | 실행 위치 | 동작 |
|--------|-----------|------|
| **Evict** | 토큰 루프 내 | `kv_caches[i].prune_prefix(remove)` — 모든 레이어에 적용 |
...
MemoryPressure(Critical)
  ...
  → generate.rs 루프:
      ...
      for cache in kv_caches:
          cache.prune_prefix(250)  // 앞쪽 250 토큰 제거
      // current_pos: 500 → 250
...
```
User            generate.rs          ResilienceManager    DbusListener      D-Bus
 │                   │                      │                   │              │
 │  --enable-res     │                      │                   │              │
 │──────────────────>│                      │                   │              │
 │                   │ mpsc::channel()      │                   │              │
