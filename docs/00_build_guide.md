...
```mermaid
flowchart LR
    S["f16 scale<br/>(2 bytes)"] --- N["[u8; 16] packed nibbles<br/>(16 bytes)"]
```
...
```mermaid
flowchart LR
    S["f16 scale<br/>(2 bytes)"] --- Q["[i8; 32] quants<br/>(32 bytes)"]
```

용도: Q4_0 matmul 가속을 위한 중간(intermediate) 포맷 — activation을 Q8_0으로 양자화 → Q4_0×Q8_0 dotprod 수행

**BlockQ4_1** (20 bytes per 32 elements):

```mermaid
flowchart LR
    S["f16 scale<br/>(2 bytes)"] --- M["f16 min<br/>(2 bytes)"] --- N["[u8; 16] packed nibbles<br/>(16 bytes)"]
```
...
flowchart TD
    IN["Input"]
    NORM1["RMSNorm"]
    QKV["QKV matmul (q_proj, k_proj, v_proj)"]
    ROPE["RoPE (positional encoding)"]
    KVC["KV Cache update"]
    ATTN["Attention"]
    OPROJ["Output projection (o_proj)"]
    RES1["Residual add"]
    NORM2["RMSNorm"]
    FFN["FFN: gate_proj → SiLU × up_proj → down_proj"]
    RES2["Residual add"]
    OUT["Output"]

    IN --> NORM1 --> QKV --> ROPE --> KVC --> ATTN --> OPROJ --> RES1
    RES1 --> NORM2 --> FFN --> RES2 --> OUT
```
...
```mermaid
flowchart LR
    ST["Safetensors (memmap)"]
    CONV["dtype 변환<br/>(BF16/F16 → F32)"]
    QUANT["선택적 Q4_0 양자화"]
    TENSOR["Tensor 생성"]

    ST --> CONV --> QUANT --> TENSOR
```
...
flowchart TD
    L["logits"]
    RP["repetition penalty 적용"]
    TS["temperature scaling"]
    SM["softmax"]
    TK["top-K filtering"]
    TP["top-P (nucleus) filtering"]
    MS["multinomial sampling"]
    NT["다음 토큰"]

    L --> RP --> TS --> SM --> TK --> TP --> MS --> NT
```
...
```mermaid
flowchart LR
    subgraph SoC["ARM SoC"]
        CPU["CPU<br/>pointer"]
        GPU["GPU<br/>cl_mem"]
        MEM[("동일 물리 메모리<br/>CL_MEM_ALLOC_HOST_PTR")]
        CPU <--> MEM <--> GPU
    end

    SoC ~~~ NOTE["memcpy 불필요 → 대역폭 절약"]
```
