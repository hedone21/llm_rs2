...
| **C12** | **live KIVI device 경로(`opencl.rs` impl·`kivi_cache`·`kivi_format`·`init`) 재작성은 host 게이트로 무회귀 증명 불가** → S25 device KIVI e2e 재증명 필수. host 4-게이트는 ABI 배선만 증명(동일 게이트가 live 회귀를 못 봄). | D8 | B 채택의 대가. device gate 전까지 "live KIVI 무회귀" 미확정. |
