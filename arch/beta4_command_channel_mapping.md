...
| 9 | `RestoreDefaults`(:240) | :484-502 | `restore_defaults`(:52) + ~~`recall_offload`(:50)~~ **삭제 (AB-3)** + throttle/tbt reset | `restore_defaults` + reset 묶음 + **`submit_offload_recall()`(AB-3 — offload 적용됐을 때만 OffloadStage::recall submit)** | throttle/tbt reset + quant guard clear + partition Full 복원 + **offload recall submit (`recall_offload`/`offload_ratio` 필드 set 제거, dispatcher `offload_armed` 게이트로 대체)** |
| 10 | `RequestQcf`(:236) | :527-530 | ~~`request_qcf: bool`(:54)~~ **삭제 (AB-5)** | **dispatcher 직결 query** — `compute_and_send_qcf()` → `qcf_runtime::compute_qcf_estimates(ctx)` → `report_tx.send(EngineMessage::QcfEstimate)`. swap directive `submit_swap`(`registry.submit`) 자리에 RequestQcf 는 직접 산출·송출(query 라 Stage 부적합 — §5.8.0). `report_tx`=adapter `report_sender()` clone(swap_runtime 동일 source). | **✅ AB-5 완료** — 자세히는 `pipeline_stage_design_v2.md §5.8`. `LoopControl.request_qcf` 필드 삭제(write-only dead-end 소멸 — set 후 소비자 0 였던 것 해소). v1 등가 anchor = `generate.rs`(d5ed71d2^) L3676-3850(`compute_qcf_estimates`). manager handshake = LuaPolicy 2-step(`lua_policy.rs:780-790`/`:871-891`) — **응답 도착 자체가 unblock**(빈 estimates 도 성사). S25 verify 4건(`signal_memory_critical`·`signal_thermal_critical_throttle`×f16/q4) RED→GREEN. |
...
**예외 — Resume(8)**: run() 본문에 직접 소비 코드가 없다. Resume 은 executor 내부 state(`engine_state`) 전이만(executor.rs:517-526). LoopControl 에 필드는 두되, β-4 에서는 등가 seam 잔존(소비 무변)으로 처리한다.

**[정오 2026-06-11 — AB-5 완료] RequestQcf(10)**: β-4 시점엔 `plan.request_qcf` 플래그를 generate 측(또는 EngineReport)이 읽어 QCF 송출하는 구조였으나, generate 분할로 v1 compute(`compute_qcf_estimates`, bin-local)가 소멸하면서 β-7 이후 **소비자 0**(`LoopControl.request_qcf` set 후 아무도 읽지 않음 = silent no-op)이 됐다. AB-5 가 이를 **dispatcher 직결 query** 로 재배선(`compute_and_send_qcf` → `report_tx.send(QcfEstimate)`, §5.8) — `LoopControl.request_qcf` 필드 삭제. 이는 KvOffload(13) 와 함께 §1.3 과도기 잔존(LayerSkip(16) 만 남음)을 줄이는 게 아니라, **②control 명령 중 silent no-op 였던 RequestQcf 의 소비자 신규 배선**(no-op 해소)이다 — AB-2 KvQuantDynamic 의 no-op 해소와 동질.
...
| 13 | `KvOffload`(:231) | :476-483 | ~~`offload_ratio: Option<f32>`(:47)~~ **삭제 (AB-3)** | **① OneShot OffloadStage** — `submit_offload(ratio)` → `OffloadStage::offload(kv_handles, cache_manager, ratio)` → `registry.submit`; `KvMutate`(eviction 과 동일 KV phase) 에서 소비(`take_inner`→`cm.offload`→`put_inner` UER, LRU prefix disk offload). sticky = **transient**(armed/last-applied 게이트 불요 — KvOffload directive 가 transient 라 매 directive = 새 submit, swap `swap_transient` 동형). **pos 환류 필요**(prune → pos 감소, eviction 환류로 자동 흡수). recall = RestoreDefaults 가 `submit_offload_recall()` → `OffloadStage::recall(..)` (offload 적용됐을 때만) | **✅ AB-3 완료** — 자세히는 `pipeline_stage_design_v2.md §5.10`. `LoopControl.offload_ratio`/`recall_offload` 필드 삭제(decode_loop (a.6) 인라인 블록 + `Forward::try_offload`/`try_recall` + dispatcher `cache_manager()` getter 소거). KV 축 Stage(`stages/kv/offload.rs`, EvictionStage 형제 — 동일 핸들·CM·UER). 로그 marker = `[Resilience] KvOffload: ratio=`/`[Resilience] Recalled {} tokens from swap`(verify YAML 글자단위 계약). **KvMode::Offload(OffloadForward) bail 은 별개 차원 — 유지** |
...
**[정오 2026-06-11 — AB-2/4/6 완료]**: 위 5종 중 **partition(15)은 AB-4 에서, swap(14)은 AB-6 에서, quant(12)는 AB-2 에서 OneShot Stage 로 이전**됐다(`LoopControl.partition_ratio`·`swap_weights`·`kv_quant_bits` 필드 3종 삭제). SwapWeights 의 v1 `take_pending_swap_weights`(executor.rs:258) live 경로는 `WeightSwapStage` commit 경로(`handle_swap_weights` §1~7 이전, §5.6.2)로 수렴. **KvQuantDynamic(12)은 v2 에서 live 소비 경로가 0**(`control.kv_quant_bits` set 후 decode_loop 미배선 = silent no-op)이었으므로 β-4 게이트의 "live 소비 등가" 대상이 아니었다 — AB-2 가 v1 등가(`generate.rs`(d5ed71d2^) L4392-4407 transition_bits loop)를 KiviQuantStage commit 으로 *신규 배선*(no-op 해소)하고, host 게이트(dispatcher sticky 단위테스트 + transition_bits 등가)로 검증한다(§5.7).

**[2026-06-13 — AB-3 완료]**: **KvOffload(13)는 AB-3 에서 OneShot OffloadStage 로 이전**됐다(`LoopControl.offload_ratio`/`recall_offload` 필드 2종 삭제 + decode_loop (a.6) 인라인 블록 + `Forward::try_offload`/`try_recall` + dispatcher `cache_manager()` getter 소거). 따라서 과도기 잔존은 **LayerSkip(16) 1종**으로 줄었다(AB-/layer-skip Stage 대기). KvOffload 는 β-4 게이트의 "live 소비 경로 보유"(decode_loop (a.6))였으므로 AB-3 가 구==신 등가(v1 `try_offload`/`try_recall` UER inner op byte-identical + verify 로그 marker 글자단위)를 보장한다. OffloadStage 는 **EvictionStage 와 동일 자원**(`Vec<Arc<StandardFormat>>` + `Arc<Mutex<CacheManager>>`)을 써 **dispatcher 생성자 시그니처 무변경**(AB-2/4/6 의 handle 추가 ripple 과 달리, §5.10.6). offload 는 pos 감소 → eviction 환류로 자동 흡수, recall 은 pos 증가 → driver `reconcile_kv_pos` 일반화(`<`→`!=` + on_kv_prune 비대칭 보존, §5.10.4)로 흡수. **KvMode::Offload(OffloadForward) bail 은 별개 차원으로 유지**(AB-3 범위 밖, §5.10).
...
| 11 | ~~`offload_ratio`(:47)~~ **삭제 (AB-3)** | (구) KvOffload(:478) / (a.6) read → 신: dispatcher `submit_offload` (transient — armed 게이트 없음) | **① OneShot OffloadStage** (LoopControl 필드 제거 — (a.6) 인라인 + `try_offload` 소거; §5.10) |
| 12 | ~~`recall_offload`(:50)~~ **삭제 (AB-3)** | (구) RestoreDefaults(:486) / (a.6) recall read → 신: dispatcher `submit_offload_recall` + `offload_armed` 게이트 | **① OneShot OffloadStage::recall** (LoopControl 필드 제거 — (a.6) 인라인 + `try_recall` 소거; §5.10.3) |
| 13 | `restore_defaults`(:52) | RestoreDefaults(:485) | ② LoopControl |
| 14 | ~~`request_qcf`(:54)~~ **삭제 (AB-5)** | (구) RequestQcf(:528) → 신: dispatcher `compute_and_send_qcf` + `report_tx` 송출 | **dispatcher 직결 query** (LoopControl 필드 제거 — write-only dead-end 소멸; §5.8.3) |
...
**검증 결론**: 19필드 중 `partition_ratio`(#15, AB-4 삭제)·`swap_weights`(#19, AB-6 삭제)·`kv_quant_bits`(#10, AB-2 삭제) 3필드가 OneShot Stage(PartitionStage/WeightSwapStage/KiviQuantStage)로 이전돼 LoopControl 에서 제거됐고, `request_qcf`(#14, AB-5 삭제)는 dispatcher 직결 query(`compute_and_send_qcf` + `report_tx`)로 이전돼 제거됐다(총 4필드 제거). 잔존 15필드는 전부 1개 이상 variant 에 매핑됨. 누락 0. (sticky carry-only 필드도 set source variant 가 존재한다 — sticky 는 같은 variant 의 재현일 뿐.)
