...
| 14 | `SwapWeights`(:209) | :562-569 | `swap_weights: Option<(f32,DtypeTag)>`(:64) | **(a.6)/decode tick — `take_pending_swap_weights`(executor.rs:258) live** | AB-6 weight-swap Stage |
| 15 | `SetPartitionRatio`(:254) | :531-535 | `partition_ratio: Option<f32>`(:56) | run() 인라인 소비 없음 — forward/transformer 측 (sticky) | AB-4 partition Stage |
...
| 15 | `partition_ratio`(:56) | SetPartitionRatio(:533) / sticky(:315) | 과도기 (LoopControl deprecated) |
...
| 19 | `swap_weights`(:64) | SwapWeights(:567) | 과도기 (LoopControl deprecated, live take_pending) |

**검증 결론**: 19/19 필드 전부 1개 이상 variant 에 매핑됨. 누락 0. (sticky carry-only 필드도 set source variant 가 존재한다 — sticky 는 같은 variant 의 재현일 뿐.)
