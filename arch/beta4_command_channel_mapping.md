...
|---|---|---|---|---|---|
| 12 | `KvQuantDynamic`(:227) | :465-475 | `kv_quant_bits: Option<u8>`(:43) | run() 인라인 소비 없음 — generate/forward 측 (sticky) | AB-2(KIVI) Stage |
...
**[정오 2026-06-11 — AB-6 완료]**: 위 5종 중 **partition(15)은 AB-4 에서, swap(14)은 AB-6 에서 OneShot Stage 로 이전**됐다(`LoopControl.partition_ratio`·`swap_weights` 필드 양쪽 삭제). 따라서 과도기 잔존은 **KvOffload(13)/KvQuantDynamic(12)/LayerSkip(16) 3종**으로 줄었다(AB-/offload·KIVI·layer-skip Stage 대기). SwapWeights 의 v1 `take_pending_swap_weights`(executor.rs:258) live 경로는 `WeightSwapStage` commit 경로(`handle_swap_weights` §1~7 이전, §5.6.2)로 수렴 — β-4 게이트의 "SwapWeights live 소비 등가"는 AB-6 host 게이트(`handle_swap_weights` §1~7 등가 + transient/in-flight 단위테스트)로 승계된다.
...
| 10 | `kv_quant_bits`(:43) | KvQuantDynamic(:467) / sticky(:312,:341) | 과도기 (LoopControl deprecated) |
...
**검증 결론**: 19필드 중 `partition_ratio`(#15, AB-4 삭제)·`swap_weights`(#19, AB-6 삭제) 2필드가 OneShot Stage(PartitionStage/WeightSwapStage)로 이전돼 LoopControl 에서 제거됐다. 잔존 17필드는 전부 1개 이상 variant 에 매핑됨. 누락 0. (sticky carry-only 필드도 set source variant 가 존재한다 — sticky 는 같은 variant 의 재현일 뿐.)
