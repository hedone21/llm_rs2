...
| `kv_roadmap_item0_measurement.md` | KV 로드맵 항목 0 측정 설계 — R-KV/A2SF/head 분산/Demote 4종 프로토콜 + 게이트 임계 고정 (2026-06-12 측정 종결: 보류/보류/개봉 후보/RED — 결과 `experiments/kv_roadmap_item0/rerun/REPORT.md`) |
| (ADR-0004 §10 M-Q) | KV 로드맵 항목 3 — `TensorKind::QueryStats` (Expected Attention query running mean/var) 설계. **별도 arch 파일 미신설** — ADR-0004 §10 amendment 가 구현 HOW(좌표계 MQ-1/GQA 환원 MQ-2/`QueryStatsAccumulator` Welford MQ-3/캡처 seam MQ-4 score-active 게이트/AbiStageCtx 5확장 MQ-5/e2e seam MQ-6 + Spec Triage spec-무관 + P2 task 분해)를 단일 SSOT 로 담는다. §7 deferred(`query_state` Quest) 를 닫음 (2026-06-12, Architect P1) |
| (ADR-0011) | KV 로드맵 항목 4 — read-plan 표면(`KVReadStage` plan-returning trait, "무엇을 읽을지" 4번째 stage 축 동작) 설계. **별도 arch 파일 미신설 — 구현 코드 0줄(리팩토링 머지 후 착수)** 이라 매핑할 구현 부재. `docs/adr/0011-kv-read-plan-surface.md` 가 표면 SSOT(`KVReadPlan{granularity,select}` D2 / 발화 시점 D3 마스크⊥prefetch / format `attention_into_selected` capability opt-in D4 / page 메타 read-stage 유지 D5 / read⊥cache plan 직교 §6 / C-ABI forward-compat §11 + Spec Triage spec-무관). 구현 단계 분해는 backlog 신규 항목으로 재등록(ADR §9). arch 컴포넌트 매핑은 구현 착수 시점에 신설(ADR-0004 §10 M-Q 의 `arch/kv_query_stats.md` 타이밍 규율 동일) (2026-06-12, Architect P4) |
