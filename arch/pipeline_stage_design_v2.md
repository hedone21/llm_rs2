...
flowchart TB
    ...
    subgraph L2["L2 engine 직속 : 확장점 trait"]
        ...
        fabric["Fabric : device 바탕 (backend, memory)"]
        kvl["KVCacheFormat"]
        wl["WeightFormat"]
...
### 3.5 Fabric — device 실행 바탕

**역할.** Format·Stage 가 *그 위에서 실행되는* 물리 자원(device)을 담는 런타임 객체. device 는 별도 축이 아니라 두 축이 올라앉는 **바탕**이다(`/CONTEXT.md` — "device는 축이 아니라 바탕"). `Backend`(§3.1)가 단일 연산기라면, `Fabric` 은 전환·분산을 위해 **여러 backend + memory** 를 묶는다.
...
```rust
struct Fabric {  // 이름 미확정 — Substrate/Placement/ExecutionContext 기각, Fabric 잠정
...
**역할.** weight layer 의 dispatch 모드(Full / Skip / Partition)를 적용하는 base trait — KV(§4.1)와 대칭이다. base-trait-handle 을 든 Stage 는 dispatch 모드만 알고 weight content 는 모른다. precision swap 등 Format mutation 은 concrete-handle Stage(예: `WeightSwapStage` with `Arc<LayerSlot>`)가 concrete method 로 직접 수행한다. `LayerDispatch::Partition` 의 분산 대상 backend(GPU + CPU companion)는 `Fabric`(§3.5)에서 resolve 하므로, **partition = dispatch 모드(Format 축) × companion backend(실행 바탕)의 곱**이다 (device 는 별도 축이 아님 — §3.5 연혁).
