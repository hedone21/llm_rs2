...
| (3) forward path generic → trait object (`LlamaLayer`/attention) | layer-tier (N×/token) | **device**: bit-identical + avg_tbt Δ≤+3% ★perf 위험 집결 |
| (4) `KVCacheOps` 폐기 | 없음 (deletion) | **device**: bit-identical + avg_tbt Δ≤+3% (parallel path 제거 후 *진짜 최종 perf*) |
...
- **avg_tbt 를 (2)/(4)에도 측정** (사용자 결정 — "가정 말고 실측", `feedback_adreno_subgroup_reduce` 정신): (3)은 regression 이 *예상 가능*(layer-tier vtable, ④-a concrete-handle 흡수 검증). (2)/(4)는 이론상 무변이라 Δ>+3% 면 **surprise → root-cause 의무**(tripwire). 특히 (4) avg_tbt 가 *진짜 최종 perf* — parallel path 제거 후라야 실제 monomorphization 이 드러남.
- **(1)만 host-only** — 순수 *추가*(수정 아님)라 host build+test 가 옛 경로 불변을 완전 커버. (1)에 forward wiring 이 섞이면 device 로 승격.
- **(3) 세부 분해**: (3)은 *각각 runnable + 개별 device-gated* 인 증분들로 쪼갠다 — **정확한 cut point 는 α-K 진입 시 type-flip ripple 을 보고 결정**(미리 고정 = speculative). 원칙(증분마다 runnable+gated)만 지금 고정 → device 게이트 실패가 (3) 안에서도 한 증분으로 격리.
- **revoke 연동**: device 게이트 실패는 ADR-0001 §6.5 revoke 조건 평가 trigger — substep 단위 조기 발견으로 4–6주 매몰 전 후퇴 가능.
