...
| (3p) **plan path** generic → trait object (`plan.rs::execute<C>` → `attention_into` 배선) | **layer-tier (N×/token) — 진짜 production hot crux** | **device**: bit-identical + avg_tbt Δ≤+3% ★perf 위험 집결 (substep (4) 선결) |
| (4) `KVCacheOps` 폐기 | 없음 (deletion) | **device**: bit-identical + avg_tbt Δ≤+3% (parallel path 제거 후 *진짜 최종 perf*) — **(3p) 선결 필수**(아래 ⚠️ plan-dep) |
...
- **avg_tbt 를 (2)/(4)에도 측정** (사용자 결정 — "가정 말고 실측", `feedback_adreno_subgroup_reduce` 정신): regression 이 *예상 가능*한 곳은 **(3p)** 다(layer-tier vtable, ④-a concrete-handle 흡수 검증). (2)/(3)/(4)는 이론상 무변이라 Δ>+3% 면 **surprise → root-cause 의무**(tripwire). 특히 (4) avg_tbt 가 *진짜 최종 perf* — parallel path 제거 후라야 실제 monomorphization 이 드러남.
...
>
> **⚠️ plan-flip = 진짜 layer-tier perf crux (신규 substep (3p), 2026-06-03).** plan path 가 production hot 인데 `plan.rs::execute<C: KVCacheOps>` 는 **여전히 `<C: KVCacheOps>` generic** 이고 attention 을 `AttentionVariant` enum static dispatch 로 처리(`attention_into` 호출 0건). 이 plan path 를 trait object 로 flip 하는 것이 진짜 production-hot layer-tier crux 다. census 이전엔 어느 substep 에도 명확히 안 들어가 있었다 → **(3p) 로 분리 신설**(별도 substep 승격이 아니라 (3) 직후·(4) 직전 선결 증분). 근거: (i) plan path 가 production hot 임이 소스로 확정(위), (ii) `attention_into` 미배선이라 (3) 와 독립한 별도 작업, (iii) (4) `KVCacheOps` 폐기가 plan flip 에 차단됨(아래 plan-dep). ④-b(`AttentionVariant` 평탄화)는 (3p) 와 한 묶음일 수 있으나 friction-triggered 로 별도 평가.
>
> **⚠️ plan-dep — (4) 차단 의존성.** ADR-0001 §8.3 substep (4) = "`KVCacheOps` 폐기" 인데, `plan.rs::execute<C: KVCacheOps>` 가 `KVCacheOps` 를 계속 generic bound 로 쓰는 한 폐기 **불가**. 따라서 **(3p) plan-flip 이 (4) 의 선결 조건**이다(plan path 가 `KVCacheOps` 의 마지막 generic 소비자). (3p) 완료 전 (4) 진입 시 컴파일 차단.
...
>   - **(3c)** 신 entry `forward_into_fmt` + 신 args struct + `ModelForward` fmt-cache wiring 으로 **decode fallback 단일 호출처**를 trait object 로 전환. ★device 게이트 — **`--no-gpu-plan` 강제 필수**(미강제 시 vacuous, 위 ⚠️) + substep (2) 교훈 eviction 발화 조건(`-n 256 --memory-threshold-mb 999999 --eviction-target-ratio 0.5`).
>   - **(3d)** prefill path flip + plan 평가(여기서 (3p) 분기 결정 — plan path 가 `KVCacheOps` 유지인지 trait flip 인지). device.
> 원칙(증분마다 runnable+gated)은 불변 — device 게이트 실패가 (3) 안에서도 한 증분으로 격리.
- **revoke 연동**: device 게이트 실패는 ADR-0001 §6.5 revoke 조건 평가 trigger — substep 단위 조기 발견으로 4–6주 매몰 전 후퇴 가능. 단 **(3) 의 게이트 실패는 cold path 한정**이라 revoke trigger 강도가 낮다(production 미영향); production perf revoke 판정의 무게는 **(3p)** 에 있다(§6.5 cold-path 반영).
