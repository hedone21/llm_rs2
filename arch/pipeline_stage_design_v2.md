...
| 새 KV 관리 Format | `KVCacheFormat` + paired attention kernel | `pressure/` + `backend/<hw>/` |
...
| `KVCacheFormat` impl + forward path | **L3** | `engine/src/pressure/` (KV format), `engine/src/layers/`·`inference/` (forward) |
| `WeightFormat` impl | **L3** | `engine/src/models/weights/` |
| `EvictionPolicy` impl | **L3** | `engine/src/pressure/eviction/` |
...
| `hardware.rs` | L2 | `Hardware`, `BackendRegistry`, `MemoryRegistry`, `DeviceTarget` | §3.5. `session/init.rs` 4 Arc 흡수. `DeviceTarget` variant = **G5** |
| `pipeline.rs` | L2 | `PipelineStage`, `LifecyclePhase`, `StageContext`, `StepInfo`, `Pressure`, `PressureSource`(trait), `StageOutcome`, `StopReason`, `StageLifecycle`, `PipelineDispatcher`(trait) | §5. `Pressure` 는 `pressure/` 이름 충돌(디렉토리 점유) + 유일 소비처가 `StepInfo` 라 여기 동거. `StepInfo` 필드 = **G5** |
| `stages/kv/{eviction,d2o,kivi_quantize,swap_dispatch,tier_move}.rs` | L3 | 각 KV-mutate Stage | 현 `pressure/*_handler.rs` 4종 + `tier_move`(신규). `pressure/` 정책·포맷에 수평 의존(L3→L3) |
| `stages/weight/weight_swap.rs` | L3 | `WeightSwapStage` | concrete-handle `Arc<LayerSlot>`(§4.2). 현 `pressure/weight_swap_handler.rs` |
...
| `pressure/`(유지) | L3 | `KVCacheFormat` impl(`kv_cache.rs`/`kivi_cache.rs`), d2o 알고리즘(`d2o_layer_alloc.rs`), `offload/` | format·알고리즘 = **데이터**. 트리거(Stage)만 `stages/` 로 분리 |
| `pressure/eviction/`(유지) | L3 | `EvictionPolicy` impl(sliding/h2o/streaming/...) | §0.4 "새 eviction 정책 → 여기". Stage 아님 |
...
| KV/Weight format (`KIVIFormat` 등) | forward + pressure | 필드 | format 생성 시 pull |
| cache (`KiviCache` 등) | pressure | 필드 | 생성 시 pull |
...
flowchart LR
    ...
    upd["update_gpu<br/>pressure 도메인"]
